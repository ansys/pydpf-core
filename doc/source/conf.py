...
if os.environ.get("BUILD_TUTORIALS", "true") != "true":
    # Exclude old RST tutorial content files (keep index.rst and gallery-generated files).
    # Detect gallery-managed subsection dirs by scanning sphinx_gallery_tutorials/.
    _sg_tutorials_root = Path(__file__).parent.parent / "sphinx_gallery_tutorials"
    _gallery_tutorial_dirs = frozenset(
        p.name for p in _sg_tutorials_root.iterdir() if p.is_dir()
    )
    for tutorial_file in glob(str(Path("tutorials") / "**" / "*.rst")):
        p = Path(tutorial_file)
        if p.name == "index.rst":
            continue
        # Files directly in tutorials/ (e.g. sg_execution_times.rst) or inside a
        # gallery-managed subdirectory are generated by sphinx-gallery — keep them.
        if p.parent.name == "tutorials" or p.parent.name in _gallery_tutorial_dirs:
            continue
        exclude_patterns.append(tutorial_file.replace("\\", "/"))
