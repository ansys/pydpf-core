...
.. note::
    ...
    instead install Tox into a dedicated environment with:

    .. code-block:: bash

        uv sync --only-group tox

    and prefix every subsequent Tox command with ``uv run``.
...
Three environment variables are available for the documentation build:
...
.. toctree::
    ...
    guidelines/guidelines_tutorials
    guidelines/guidelines_examples