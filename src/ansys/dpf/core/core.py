...
class BaseService:
    ...
    def _connect(self, timeout=5):
        ...
        if timeout is not None:
            ...
            if not state._matured:
                raise IOError(f"Unable to connect to DPF instance at {self._server()._address}")
    ...
    def download_file(self, server_file_path, to_client_file_path):
        ...
        for i in range(0, len(chunks.initial_metadata())):
            if chunks.initial_metadata()[i].key == "size_tot":
                ...
                bar = _common_progress_bar("Downloading...", unit="KB", tot_size=tot_size)
        if not bar:
        ...
        for chunk in chunks:
            if chunk.data.server_file_path != server_path:
                ...
                if (
                    ...
                    server_subpath = server_path.replace(str(server_folder_path) + separator, "")
                    subdir = ""
    ...
    def _upload_and_get_server_path(
        ...
        if ((specific_extension is not None) and (f.endswith(specific_extension))) or (
            ...
            server_path = self._stub.UploadFile(
                self.__file_chunk_yielder(file_path=f, to_server_file_path=to_server_file_path)
            ).server_file_path
    ...
    _to_cache = {_get_server_info: None}
