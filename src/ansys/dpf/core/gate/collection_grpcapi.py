...
from ansys.dpf.core.gate import object_handler, data_processing_grpcapi, grpc_stream_helpers, errors
...
class CollectionGRPCAPI(collection_abstract_api.CollectionAbstractAPI):
    ...
    def init_collection_environment(object):
        from ansys.dpf.core.grpc.dpf import collection_pb2, collection_pb2_grpc
        ...
        object._deleter_func = (
            _get_stub(server).Delete,
            lambda obj: obj._internal_obj if isinstance(obj, collection_pb2.Collection) else None,
        )
    ...
    def collection_new_on_client(client, type):
        from ansys.dpf.core.grpc.dpf import collection_pb2
    ...
    def _collection_get_entries(collection, label_space_or_index):
        from ansys.dpf.core.grpc.dpf import (
            collection_pb2,
            scoping_pb2,
            field_pb2,
            meshed_region_pb2,
        ...
        for obj in out.entries:
            ...
            if obj.HasField("dpf_type"):
                if collection._internal_obj.type == base_pb2.Type.Value("SCOPING"):
                    entry = object_handler.ObjHandler(
                        data_processing_grpcapi.DataProcessingGRPCAPI, scoping_pb2.Scoping()
                    )
                elif collection._internal_obj.type == base_pb2.Type.Value("FIELD"):
                    entry = object_handler.ObjHandler(
                        data_processing_grpcapi.DataProcessingGRPCAPI, field_pb2.Field()
                    )
    ...
    def _collection_set_data_as_integral_type(collection, data, size):
        ...
        request.collection.CopyFrom(collection._internal_obj)
        _get_stub(collection._server).UpdateAllData(
    ...
    def collection_get_support(collection, label):
        from ansys.dpf.core.grpc.dpf import collection_pb2, base_pb2
    ...
    def collection_get_data_as_int(collection, size):
        ...
        data_type = "int"
        dtype = np.int32
        service = _get_stub(collection._server).GetAllData(
            request, metadata=[("float_or_double", data_type)]
        )
