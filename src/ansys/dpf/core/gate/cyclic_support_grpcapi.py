from ansys.dpf.core.gate.generated import cyclic_support_abstract_api
from ansys.dpf.core.gate.data_processing_grpcapi import DataProcessingGRPCAPI
from ansys.dpf.core.gate.object_handler import ObjHandler
...
class CyclicSupportGRPCAPI(cyclic_support_abstract_api.CyclicSupportAbstractAPI):
    ...
    def init_cyclic_support_environment(support):
        from ansys.dpf.core.grpc.dpf import cyclic_support_pb2_grpc
    ...
    def list(support):
        ...
        for i_stage in range(list_response.num_stages):
            ...
            setattr(
                response,
                "base_elements_scoping_" + str(i_stage),
                ObjHandler(api, list_response.stage_infos[i_stage].base_elements_scoping, server),
            )
            setattr(
                response,
                "base_nodes_scoping_" + str(i_stage),
                ObjHandler(api, list_response.stage_infos[i_stage].base_nodes_scoping, server),
            )
            setattr(
                response,
                "sectors_for_expansion_" + str(i_stage),
                ObjHandler(api, list_response.stage_infos[i_stage].sectors_for_expansion, server),
            )
        return response

    @staticmethod
    def cyclic_support_get_num_sectors(support, i_stage):
        return getattr(CyclicSupportGRPCAPI.list(support), "num_sectors_" + str(i_stage))

    @staticmethod
    def cyclic_support_get_num_stages(support):
        return getattr(CyclicSupportGRPCAPI.list(support), "num_stages")

    @staticmethod
    def cyclic_support_get_sectors_scoping(support, i_stage):
        return getattr(
            CyclicSupportGRPCAPI.list(support), "sectors_for_expansion_" + str(i_stage)
        ).get_ownership()

    @staticmethod
    def cyclic_support_get_base_nodes_scoping(support, i_stage):
        return getattr(
            CyclicSupportGRPCAPI.list(support), "base_nodes_scoping_" + str(i_stage)
        ).get_ownership()

    @staticmethod
    def cyclic_support_get_base_elements_scoping(support, i_stage):
        return getattr(
            CyclicSupportGRPCAPI.list(support), "base_elements_scoping_" + str(i_stage)
        ).get_ownership()
    ...
    def init_get_expanded_ids(support, i_stage, sectorsScoping):
        from ansys.dpf.core.grpc.dpf import cyclic_support_pb2
