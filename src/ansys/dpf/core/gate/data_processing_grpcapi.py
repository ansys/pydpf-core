...
from ansys.dpf.core.gate import errors, object_handler, misc
...
def _get_stub(server):
    from ansys.dpf.core.grpc.dpf import base_pb2_grpc
...
def _get_server_info_response(client):
    from ansys.dpf.core.grpc.dpf import base_pb2
...
class DataProcessingGRPCAPI(data_processing_abstract_api.DataProcessingAbstractAPI):
    ...
    def init_data_processing_environment(object):
        ...
        if not hasattr(object, "_server"):
            object.create_stub_if_necessary(
                DataProcessingGRPCAPI.STUBNAME, base_pb2_grpc.BaseServiceStub
            )
    ...
    def bind_delete_server_func(object):
        ...
        if hasattr(_get_stub(object), "ReleaseServer"):
            object._shutdown_func = (_get_stub(object).ReleaseServer, base_pb2.Empty())
        else:
            object._shutdown_func = (None, None)
    ...
    def data_processing_delete_shared_object(data):
        if not data._server.meet_version("4.0"):
            ...
            from ansys.dpf.core.grpc.dpf import scoping_pb2
            from ansys.dpf.core.gate import scoping_grpcapi
    ...
    def data_processing_description_string(data):
        try:
            ...
            from ansys.dpf.core.grpc.dpf import base_pb2, collection_pb2
            ...
            if isinstance(data_obj, collection_pb2.Collection):
                from ansys.dpf.core.gate import collection_grpcapi
    ...
    def data_processing_list_operators_as_collection_on_client(client):
        from ansys.dpf.core.grpc.dpf import operator_pb2
        from ansys.dpf.core.gate import operator_grpcapi
    ...
    def data_processing_get_global_config_as_data_tree_on_client(client):
        from ansys.dpf.core.grpc.dpf import base_pb2, data_tree_pb2
    ...
    def data_processing_download_file(client, server_file_path, to_client_file_path):
        ...
        from ansys.dpf.core.gate import misc
        ...
        for i in range(0, len(chunks.initial_metadata())):
            if chunks.initial_metadata()[i].key == "size_tot":
                tot_size = int(chunks.initial_metadata()[i].value) * 1e-3
                if _progress_bar_is_available:
                    bar = misc.COMMON_PROGRESS_BAR("Downloading...", unit="KB", tot_size=tot_size)
            ...
            if chunk.data.server_file_path != server_path:
                ...
                if (
                    ...
                    if s2 > s1:
                        ...
                        separator = "\\"
                    server_subpath = server_path.replace(server_file_path + separator, "")
                    subdir = ""
