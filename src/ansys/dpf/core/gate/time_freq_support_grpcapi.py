...
class TimeFreqSupportGRPCAPI(time_freq_support_abstract_api.TimeFreqSupportAbstractAPI):
    ...
    def _cumulative_index_request(bIsComplex, timeFreq, freq):
        from ansys.dpf.core.grpc.dpf import time_freq_support_pb2
    ...
    def _copy_into(request, time_freq_support):
        from ansys.dpf.core.grpc.dpf import time_freq_support_pb2, support_pb2
    ...
    def init_time_freq_support_environment(obj):
        from ansys.dpf.core.grpc.dpf import time_freq_support_pb2_grpc
    ...
    def list(support, stage_num=None):
        ...
        response = SimpleNamespace(
            ...
            cyclic_harmonic_index_scoping=ObjHandler(
                api, list_response.cyclic_harmonic_index_scoping, server
            )
            if hasattr(list_response, "cyclic_harmonic_index_scoping")
            else None,
        )
        return response
    ...
    def time_freq_support_get_number_sets(timeFreq):
        from ansys.dpf.core.grpc.dpf import time_freq_support_pb2, base_pb2
    ...
    def time_freq_support_get_shared_harmonic_indices(timeFreq, stage):
        return TimeFreqSupportGRPCAPI.list(
            timeFreq, stage_num=stage
        ).cyc_harmonic_index.get_ownership()
