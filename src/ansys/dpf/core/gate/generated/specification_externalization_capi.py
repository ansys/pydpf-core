...
from ansys.dpf.core.gate.generated import specification_externalization_abstract_api
...
class SpecificationExternalizationCAPI(
    specification_externalization_abstract_api.SpecificationExternalizationAbstractAPI
):
    @staticmethod
    ...
    def specification_xml_export(filepath):
        ...
        res = capi.dll.Specification_xml_export(
    ...
    def specification_xml_import(filepath):
        errorSize = ctypes.c_int(0)
        sError = ctypes.c_wchar_p()
        res = capi.dll.Specification_xml_import(
            utils.to_char_ptr(filepath),
            ctypes.byref(utils.to_int32(errorSize)),
            ctypes.byref(sError),
        )
    ...
    def set_specification_in_core(registry):
        errorSize = ctypes.c_int(0)
        sError = ctypes.c_wchar_p()
        res = capi.dll.setSpecificationInCore(
            registry, ctypes.byref(utils.to_int32(errorSize)), ctypes.byref(sError)
        )
        if errorSize.value != 0:
            raise errors.DPFServerException(sError.value)
        return res
