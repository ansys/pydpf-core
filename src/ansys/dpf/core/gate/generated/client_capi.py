...
from ansys.dpf.core.gate.generated import client_abstract_api
...
class ClientCAPI(client_abstract_api.ClientAbstractAPI):
    ...
    def client_new(ip, port):
        ...
        res = capi.dll.Client_new(
            utils.to_char_ptr(ip),
            utils.to_char_ptr(port),
    ...
    def client_new_full_address(address):
        errorSize = ctypes.c_int(0)
        sError = ctypes.c_wchar_p()
        res = capi.dll.Client_new_full_address(
            utils.to_char_ptr(address),
            ctypes.byref(utils.to_int32(errorSize)),
            ctypes.byref(sError),
        )
        if errorSize.value != 0:
    ...
    def client_get_full_address(client):
        errorSize = ctypes.c_int(0)
        sError = ctypes.c_wchar_p()
        res = capi.dll.Client_get_full_address(
    ...
    def client_get_protocol_name(client):
        errorSize = ctypes.c_int(0)
        sError = ctypes.c_wchar_p()
        res = capi.dll.Client_get_protocol_name(
            client._internal_obj if client is not None else None,
            ctypes.byref(utils.to_int32(errorSize)),
            ctypes.byref(sError),
        )
