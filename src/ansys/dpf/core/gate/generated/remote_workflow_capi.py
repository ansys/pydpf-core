...
from ansys.dpf.core.gate.generated import remote_workflow_abstract_api
...
class RemoteWorkflowCAPI(remote_workflow_abstract_api.RemoteWorkflowAbstractAPI):
    ...
    def remote_workflow_new(streams, remoteId):
        ...
        res = capi.dll.RemoteWorkflow_new(
    ...
    def remote_workflow_get_workflow_id(remote_wf):
        errorSize = ctypes.c_int(0)
        sError = ctypes.c_wchar_p()
        res = capi.dll.RemoteWorkflow_get_workflow_id(
    ...
    def remote_workflow_get_streams(remote_wf):
        errorSize = ctypes.c_int(0)
        sError = ctypes.c_wchar_p()
        res = capi.dll.RemoteWorkflow_get_streams(
            remote_wf._internal_obj if remote_wf is not None else None,
            ctypes.byref(utils.to_int32(errorSize)),
            ctypes.byref(sError),
        )
    ...
    def remote_work_flow_delete(remote_wf):
        errorSize = ctypes.c_int(0)
        sError = ctypes.c_wchar_p()
        res = capi.dll.RemoteWorkFlow_delete(
            remote_wf._internal_obj if remote_wf is not None else None,
            ctypes.byref(utils.to_int32(errorSize)),
            ctypes.byref(sError),
        )
