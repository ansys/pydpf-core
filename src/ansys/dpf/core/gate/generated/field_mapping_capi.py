...
from ansys.dpf.core.gate.generated import field_mapping_abstract_api
...
class FieldMappingCAPI(field_mapping_abstract_api.FieldMappingAbstractAPI):
    ...
    def mapping_map(obj, in_field):
        errorSize = ctypes.c_int(0)
        sError = ctypes.c_wchar_p()
        res = capi.dll.Mapping_Map(
            obj._internal_obj if obj is not None else None,
            in_field._internal_obj if in_field is not None else None,
            ctypes.byref(utils.to_int32(errorSize)),
            ctypes.byref(sError),
        )
        if errorSize.value != 0:
