...
class ResultInfoGRPCAPI(result_info_abstract_api.ResultInfoAbstractAPI):
    ...
    def init_result_info_environment(result_info):
        from ansys.dpf.core.grpc.dpf import result_info_pb2_grpc

        result_info._server.create_stub_if_necessary(
            ResultInfoGRPCAPI.STUBNAME, result_info_pb2_grpc.ResultInfoServiceStub
        )
        result_info._deleter_func = (
            _get_stub(result_info._server).Delete,
    ...
    def list(result_info):
        ...
        setattr(
            response, "cyc_support", ObjHandler(api, list_response.cyc_info.cyc_support, server)
        )
    ...
    def list_result(result_info, idx):
        from ansys.dpf.core.grpc.dpf import result_info_pb2
    ...
    def result_info_get_unit_system_name(result_info):
        if result_info._server.meet_version("3.0"):
            return ResultInfoGRPCAPI.result_info_get_string_property(
