...
class total_strain_principal_1(Operator):
    r"""Read/compute element nodal component total strains 1st principal
    ...
    >>> op = dpf.operators.result.total_strain_principal_1()
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config=config,
            server=server,
            inputs_type=InputsTotalStrainPrincipal1,
            outputs_type=OutputsTotalStrainPrincipal1,
        )
    ...
    def _spec() -> Specification:
        description = r"""Read/compute element nodal component total strains 1st principal
component by calling the readers defined by the datasources and
computing its eigen values. This operation is independent of the
coordinate system unless averaging across elements is requested, in
which case a rotation to the global coordinate system is performed. The
...
"""
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="EL1", server=server)

    @property
    def inputs(self) -> InputsTotalStrainPrincipal1:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsTotalStrainPrincipal1.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsTotalStrainPrincipal1:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsTotalStrainPrincipal1.
        """
        return self._outputs


class InputsTotalStrainPrincipal1(_Inputs):
    """Intermediate class used to connect user inputs to
    total_strain_principal_1 operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(total_strain_principal_1._spec().inputs, op)
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            total_strain_principal_1._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping: Input[ScopingsContainer | Scoping] = Input(
            total_strain_principal_1._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._mesh_scoping)
        self._fields_container: Input[FieldsContainer] = Input(
            total_strain_principal_1._spec().input_pin(2), 2, op, -1
        )
        self._inputs.append(self._fields_container)
        self._streams_container: Input[StreamsContainer] = Input(
            total_strain_principal_1._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            total_strain_principal_1._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._bool_rotate_to_global: Input[bool] = Input(
            total_strain_principal_1._spec().input_pin(5), 5, op, -1
        )
        self._inputs.append(self._bool_rotate_to_global)
        self._mesh: Input[MeshedRegion | MeshesContainer] = Input(
            total_strain_principal_1._spec().input_pin(7), 7, op, -1
        )
        self._inputs.append(self._mesh)
        self._requested_location: Input[str] = Input(
            total_strain_principal_1._spec().input_pin(9), 9, op, -1
        )
        self._inputs.append(self._requested_location)
        self._read_cyclic: Input[int] = Input(
            total_strain_principal_1._spec().input_pin(14), 14, op, -1
        )
        self._inputs.append(self._read_cyclic)
        self._read_beams: Input[bool] = Input(
            total_strain_principal_1._spec().input_pin(22), 22, op, -1
        )
...
class OutputsTotalStrainPrincipal1(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(total_strain_principal_1._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            total_strain_principal_1._spec().output_pin(0), 0, op
        )
