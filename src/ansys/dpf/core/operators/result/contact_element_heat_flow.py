...
class contact_element_heat_flow(Operator):
    r"""Read/compute contact element heat flow by calling the readers defined by
    the datasources.
    ...
    >>> op = dpf.operators.result.contact_element_heat_flow()
    ...
    def __init__(
        ...
        super().__init__(
            name="ContactElementHeatFlow",
            config=config,
            server=server,
            inputs_type=InputsContactElementHeatFlow,
            outputs_type=OutputsContactElementHeatFlow,
        )
    ...
    def _spec() -> Specification:
        description = r"""Read/compute contact element heat flow by calling the readers defined by
the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="ContactElementHeatFlow", server=server)

    @property
    def inputs(self) -> InputsContactElementHeatFlow:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsContactElementHeatFlow.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsContactElementHeatFlow:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsContactElementHeatFlow.
        """
        return self._outputs


class InputsContactElementHeatFlow(_Inputs):
    """Intermediate class used to connect user inputs to
    contact_element_heat_flow operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(contact_element_heat_flow._spec().inputs, op)
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            contact_element_heat_flow._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping: Input[ScopingsContainer | Scoping] = Input(
            contact_element_heat_flow._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._mesh_scoping)
        self._fields_container: Input[FieldsContainer] = Input(
            contact_element_heat_flow._spec().input_pin(2), 2, op, -1
        )
        self._inputs.append(self._fields_container)
        self._streams_container: Input[StreamsContainer] = Input(
            contact_element_heat_flow._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            contact_element_heat_flow._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._mesh: Input[MeshedRegion | MeshesContainer] = Input(
            contact_element_heat_flow._spec().input_pin(7), 7, op, -1
        )
...
class OutputsContactElementHeatFlow(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(contact_element_heat_flow._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            contact_element_heat_flow._spec().output_pin(0), 0, op
        )
