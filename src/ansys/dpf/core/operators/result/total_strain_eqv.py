...
class total_strain_eqv(Operator):
    r"""Read/compute element nodal equivalent total strain by calling the
    ...
    >>> op = dpf.operators.result.total_strain_eqv()
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config=config,
            server=server,
            inputs_type=InputsTotalStrainEqv,
            outputs_type=OutputsTotalStrainEqv,
        )
    ...
    def _spec() -> Specification:
        description = r"""Read/compute element nodal equivalent total strain by calling the
readers defined by the datasources. - The ‘requested_location’ and
‘mesh_scoping’ inputs are processed to see if they need scoping
transposition or result averaging. The resulting output fields have a
‘Nodal’, ‘ElementalNodal’ or ‘Elemental’ location. - Once the need for
...
"""
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="EL_EQV", server=server)

    @property
    def inputs(self) -> InputsTotalStrainEqv:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsTotalStrainEqv.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsTotalStrainEqv:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsTotalStrainEqv.
        """
        return self._outputs


class InputsTotalStrainEqv(_Inputs):
    """Intermediate class used to connect user inputs to
    total_strain_eqv operator.
...
class OutputsTotalStrainEqv(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(total_strain_eqv._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            total_strain_eqv._spec().output_pin(0), 0, op
        )
