...
class total_strain_max_shear(Operator):
    ...
    >>> op = dpf.operators.result.total_strain_max_shear()
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config=config,
            server=server,
            inputs_type=InputsTotalStrainMaxShear,
            outputs_type=OutputsTotalStrainMaxShear,
        )
...
"""
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="EL_max_shear", server=server)

    @property
    def inputs(self) -> InputsTotalStrainMaxShear:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsTotalStrainMaxShear.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsTotalStrainMaxShear:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsTotalStrainMaxShear.
        """
        return self._outputs


class InputsTotalStrainMaxShear(_Inputs):
    """Intermediate class used to connect user inputs to
    total_strain_max_shear operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(total_strain_max_shear._spec().inputs, op)
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            total_strain_max_shear._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping: Input[ScopingsContainer | Scoping] = Input(
            total_strain_max_shear._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._mesh_scoping)
        self._fields_container: Input[FieldsContainer] = Input(
            total_strain_max_shear._spec().input_pin(2), 2, op, -1
        )
        self._inputs.append(self._fields_container)
        self._streams_container: Input[StreamsContainer] = Input(
            total_strain_max_shear._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            total_strain_max_shear._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._bool_rotate_to_global: Input[bool] = Input(
            total_strain_max_shear._spec().input_pin(5), 5, op, -1
        )
        self._inputs.append(self._bool_rotate_to_global)
        self._mesh: Input[MeshedRegion | MeshesContainer] = Input(
            total_strain_max_shear._spec().input_pin(7), 7, op, -1
        )
        self._inputs.append(self._mesh)
        self._requested_location: Input[str] = Input(
            total_strain_max_shear._spec().input_pin(9), 9, op, -1
        )
        self._inputs.append(self._requested_location)
        self._read_cyclic: Input[int] = Input(
            total_strain_max_shear._spec().input_pin(14), 14, op, -1
        )
        self._inputs.append(self._read_cyclic)
        self._read_beams: Input[bool] = Input(
            total_strain_max_shear._spec().input_pin(22), 22, op, -1
        )
...
class OutputsTotalStrainMaxShear(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(total_strain_max_shear._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            total_strain_max_shear._spec().output_pin(0), 0, op
        )
