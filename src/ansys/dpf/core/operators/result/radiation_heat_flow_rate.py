...
class radiation_heat_flow_rate(Operator):
    r"""Read/compute radiation heat flow rate by calling the readers defined by
    the datasources.
    ...
    >>> op = dpf.operators.result.radiation_heat_flow_rate()
    ...
    def __init__(
        ...
        super().__init__(
            name="RadiationHeatFlowRate",
            config=config,
            server=server,
            inputs_type=InputsRadiationHeatFlowRate,
            outputs_type=OutputsRadiationHeatFlowRate,
        )
    ...
    def _spec() -> Specification:
        description = r"""Read/compute radiation heat flow rate by calling the readers defined by
the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="RadiationHeatFlowRate", server=server)

    @property
    def inputs(self) -> InputsRadiationHeatFlowRate:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsRadiationHeatFlowRate.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsRadiationHeatFlowRate:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsRadiationHeatFlowRate.
        """
        return self._outputs


class InputsRadiationHeatFlowRate(_Inputs):
    """Intermediate class used to connect user inputs to
    radiation_heat_flow_rate operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(radiation_heat_flow_rate._spec().inputs, op)
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            radiation_heat_flow_rate._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping: Input[ScopingsContainer | Scoping] = Input(
            radiation_heat_flow_rate._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._mesh_scoping)
        self._fields_container: Input[FieldsContainer] = Input(
            radiation_heat_flow_rate._spec().input_pin(2), 2, op, -1
        )
        self._inputs.append(self._fields_container)
        self._streams_container: Input[StreamsContainer] = Input(
            radiation_heat_flow_rate._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            radiation_heat_flow_rate._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._mesh: Input[MeshedRegion | MeshesContainer] = Input(
            radiation_heat_flow_rate._spec().input_pin(7), 7, op, -1
        )
...
class OutputsRadiationHeatFlowRate(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(radiation_heat_flow_rate._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            radiation_heat_flow_rate._spec().output_pin(0), 0, op
        )
