...
class grpc_start_server(Operator):
    ...
    start_in_new_thread : bool, optional
        Default is true. with false, this thread will
        be waiting for grpc calls and will
        not be usable for anything else
    ...
    >>> my_start_in_new_thread = bool()
    >>> op.inputs.start_in_new_thread.connect(my_start_in_new_thread)
    ...
    ...     ip=my_ip,
    ...     port=my_port,
    ...     start_in_new_thread=my_start_in_new_thread,
    ...
    def __init__(
        ...
        if start_in_new_thread is not None:
            self.inputs.start_in_new_thread.connect(start_in_new_thread)
    ...
    def _spec():
        ...
        is taken""",
                ...
                2: PinSpecification(
                    name="start_in_new_thread",
                    type_names=["bool"],
                    optional=True,
                    document="""Default is true. with false, this thread will
        be waiting for grpc calls and will
        not be usable for anything else""",
                ),
                3: PinSpecification(
...
class InputsGrpcStartServer(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._start_in_new_thread = Input(grpc_start_server._spec().input_pin(2), 2, op, -1)
        self._inputs.append(self._start_in_new_thread)
        self._should_start_server = Input(grpc_start_server._spec().input_pin(3), 3, op, -1)
    ...
    def start_in_new_thread(self):
        """Allows to connect start_in_new_thread input to the operator.
        ...
        >>> op.inputs.start_in_new_thread.connect(my_start_in_new_thread)
        >>> # or
        >>> op.inputs.start_in_new_thread(my_start_in_new_thread)
        """
