...
class min_over_time_by_entity(Operator):
    ...
    the per-entity, per-component minimum across all time or frequency
    steps.

    The result forwarded on output pin 0 is pin 0 of
    ``min_max_over_time_by_entity`` (the ``min`` fields container), with the
    same shape: one value per entity, per component and per shell layer when
    available.

    **When to use:** you only need the per-entity minimum over time. Prefer
    ``min_max_over_time_by_entity`` directly when you also need the maximum,
    the time of extremum, or the incremental behaviour on multiple output
    pins.
        ...
        spec = Specification(
            ...
            map_output_pin_spec={
                0: PinSpecification(
                    ...
                    type_names=["fields_container"],
                    optional=False,
                    document=r"""Per-entity, per-component minimum across all time or frequency steps. Same shape as pin 0 of `min_max_over_time_by_entity`.""",
                ),
            },
        )
        return spec
