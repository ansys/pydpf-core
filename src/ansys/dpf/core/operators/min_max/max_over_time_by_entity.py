...
class max_over_time_by_entity(Operator):
    ...
    the per-entity, per-component maximum across all time or frequency
    steps.

    The result forwarded on output pin 0 is pin 1 of
    ``min_max_over_time_by_entity`` (the ``max`` fields container), with the
    same shape: one value per entity, per component and per shell layer when
    available.

    **When to use:** you only need the per-entity maximum over time. Prefer
    ``min_max_over_time_by_entity`` directly when you also need the minimum,
        ...
        spec = Specification(
            ...
            map_output_pin_spec={
                0: PinSpecification(
                    ...
                    type_names=["fields_container"],
                    optional=False,
                    document=r"""Per-entity, per-component maximum across all time or frequency steps. Same shape as pin 1 of `min_max_over_time_by_entity`.""",
                ),
            },
        )
        return spec
