...
class create_mech_point_cloud_workflow(Operator):
    """Prepares a workflow able to map data from an input mesh to a target
    mesh.

    Parameters
    ----------
    source_mesh : MeshedRegion or Field, optional
        Source mesh. can be a meshed region or a node
        coordinates field.
    target_mesh : MeshedRegion or Field, optional
    ...
    >>> op = dpf.operators.mapping.create_mech_point_cloud_workflow()
    ...
    def __init__(
        ...
        super().__init__(name="mech_point_cloud_wf", config=config, server=server)
        self._inputs = InputsCreateMechPointCloudWorkflow(self)
        self._outputs = OutputsCreateMechPointCloudWorkflow(self)
    ...
    def default_config(server=None):
        ...
        """
        return Operator.default_config(name="mech_point_cloud_wf", server=server)
    ...
    def inputs(self):
        ...
        Returns
        --------
        inputs : InputsCreateMechPointCloudWorkflow
        """
        return super().inputs
    ...
    def outputs(self):
        ...
        Returns
        --------
        outputs : OutputsCreateMechPointCloudWorkflow
        """
        return super().outputs


class InputsCreateMechPointCloudWorkflow(_Inputs):
    """Intermediate class used to connect user inputs to
    create_mech_point_cloud_workflow operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(create_mech_point_cloud_workflow._spec().inputs, op)
        self._source_mesh = Input(
            create_mech_point_cloud_workflow._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._source_mesh)
        self._target_mesh = Input(
            create_mech_point_cloud_workflow._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._target_mesh)
        self._target_scoping = Input(
            create_mech_point_cloud_workflow._spec().input_pin(6), 6, op, -1
        )
        self._inputs.append(self._target_scoping)
        self._options_data_tree = Input(
            create_mech_point_cloud_workflow._spec().input_pin(10), 10, op, -1
        )
...
class OutputsCreateMechPointCloudWorkflow(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(create_mech_point_cloud_workflow._spec().outputs, op)
        self._mapping_workflow = Output(
            create_mech_point_cloud_workflow._spec().output_pin(0), 0, op
        )
