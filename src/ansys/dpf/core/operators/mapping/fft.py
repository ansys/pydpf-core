...
class fft(Operator):
    """Computes the Fast Fourier Transform on each component of input Field or each field of input Fields Container (you can use transpose_fields_container to have relevant scoping). Fields are assumed with the same scoping, number of components and representing equally spaced data, ideally resampled to have a 2^n points (prepare_sampling_fft with time_freq_interpolation can help creating these fields). If Complex label is present, Complex to Complex FFT performed otherwise Real to Complex is performed (only half of the coefficient will be returned).
      ...
      >>> op = dpf.operators.mapping.fft(field=my_field,scale_forward_transform=my_scale_forward_transform,inplace=my_inplace,force_fft_points=my_force_fft_points,cutoff_frequency=my_cutoff_frequency,scale_right_amplitude=my_scale_right_amplitude)

      >>> # Get output data
      >>> result_fields_container = op.outputs.fields_container()"""
    def __init__(self, field=None, scale_forward_transform=None, inplace=None, force_fft_points=None, cutoff_frequency=None, scale_right_amplitude=None, config=None, server=None):
        super().__init__(name="fft", config = config, server = server)
        self._inputs = InputsFft(self)
        self._outputs = OutputsFft(self)
        if field !=None:
            self.inputs.field.connect(field)
        if scale_forward_transform !=None:
            self.inputs.scale_forward_transform.connect(scale_forward_transform)
        if inplace !=None:
            self.inputs.inplace.connect(inplace)
        if force_fft_points !=None:
        ...
        if scale_right_amplitude !=None:
            self.inputs.scale_right_amplitude.connect(scale_right_amplitude)

    @staticmethod
    def _spec():
        spec = Specification(description="""Computes the Fast Fourier Transform on each component of input Field or each field of input Fields Container (you can use transpose_fields_container to have relevant scoping). Fields are assumed with the same scoping, number of components and representing equally spaced data, ideally resampled to have a 2^n points (prepare_sampling_fft with time_freq_interpolation can help creating these fields). If Complex label is present, Complex to Complex FFT performed otherwise Real to Complex is performed (only half of the coefficient will be returned).""",
                             map_input_pin_spec={
                                 0 : PinSpecification(name = "field", type_names=["field","fields_container"], optional=False, document="""Field or Fields Container."""), 
                                 3 : PinSpecification(name = "scale_forward_transform", type_names=["double"], optional=True, document="""Scale for Forward Transform, default is 2/field_num_elementary_data."""), 
                                 4 : PinSpecification(name = "inplace", type_names=["bool"], optional=True, document="""True if inplace, default is false."""), 
                                 5 : PinSpecification(name = "force_fft_points", type_names=["int32"], optional=True, document="""Explicitely define number of fft points to either rescope or perform zero padding."""), 
                                 6 : PinSpecification(name = "cutoff_frequency", type_names=["double"], optional=True, document="""Restrict output frequency up to this cutoff frequency"""), 
                                 7 : PinSpecification(name = "scale_right_amplitude", type_names=["bool"], optional=True, document="""If set to true (default is false), 2/field_num_entities scaling will be applied, to have right amplitude values.""")},
                             map_output_pin_spec={
                                 0 : PinSpecification(name = "fields_container", type_names=["fields_container"], optional=False, document="""Output Complex Fields Container with labels matching input Fields Container. No supports binded, but prepare_sampling_fft provides it.""")})
        return spec


    @staticmethod
    def default_config():
        return Operator.default_config(name = "fft")
...
class InputsFft(_Inputs):
    ...
    def field(self):
        ...
        - pindoc: Field or Fields Container.
    ...
    def scale_forward_transform(self):
        ...
        - pindoc: Scale for Forward Transform, default is 2/field_num_elementary_data.
    ...
    def inplace(self):
        ...
        - pindoc: True if inplace, default is false.
    ...
    def force_fft_points(self):
        ...
        - pindoc: Explicitely define number of fft points to either rescope or perform zero padding.
    ...
    def cutoff_frequency(self):
        ...
        - pindoc: Restrict output frequency up to this cutoff frequency
    ...
    def scale_right_amplitude(self):
        ...
        - pindoc: If set to true (default is false), 2/field_num_entities scaling will be applied, to have right amplitude values.
...
class OutputsFft(_Outputs):
    ...
    def fields_container(self):
        ...
        - pindoc: Output Complex Fields Container with labels matching input Fields Container. No supports binded, but prepare_sampling_fft provides it.
