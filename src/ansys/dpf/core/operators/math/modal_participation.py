...
class modal_participation(Operator):
    """Compute the modal participation factor for a given vector field V, defined as  sum_i ( V .dot. mode_shape_i * ponderation ).
      ...
      >>> op = dpf.operators.math.modal_participation(v_real=my_v_real,v_imag=my_v_imag,mode_shapes=my_mode_shapes,ponderation=my_ponderation,force_label_space=my_force_label_space)

      >>> # Get output data
      >>> result_output = op.outputs.output()"""
    def __init__(self, v_real=None, v_imag=None, mode_shapes=None, ponderation=None, force_label_space=None, config=None, server=None):
        super().__init__(name="harmonic::modal_participation", config = config, server = server)
        ...
        if force_label_space !=None:
            self.inputs.force_label_space.connect(force_label_space)

    @staticmethod
    def _spec():
        spec = Specification(description="""Compute the modal participation factor for a given vector field V, defined as  sum_i ( V .dot. mode_shape_i * ponderation ).""",
                             map_input_pin_spec={
                                 0 : PinSpecification(name = "v_real", type_names=["field"], optional=False, document="""real part of field V"""), 
                                 1 : PinSpecification(name = "v_imag", type_names=["field"], optional=False, document="""imag part of field V"""), 
                                 2 : PinSpecification(name = "mode_shapes", type_names=["fields_container"], optional=False, document=""""""), 
                                 3 : PinSpecification(name = "ponderation", type_names=["field"], optional=True, document=""""""), 
                                 4 : PinSpecification(name = "force_label_space", type_names=["label_space"], optional=True, document="""If set, will force a label space for output result.""")},
                             map_output_pin_spec={
                                 0 : PinSpecification(name = "output", type_names=["fields_container"], optional=False, document="""""")})
        return spec


    @staticmethod
    def default_config():
        return Operator.default_config(name = "harmonic::modal_participation")
...
class InputsModalParticipation(_Inputs):
    """Intermediate class used to connect user inputs to modal_participation operator
    ...
    def __init__(self, op: Operator):
        ...
        self._force_label_space = Input(modal_participation._spec().input_pin(4), 4, op, -1) 
    ...
    def force_label_space(self):
        ...
        - pindoc: If set, will force a label space for output result.
...
class OutputsModalParticipation(_Outputs):
    """Intermediate class used to get outputs from modal_participation operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
