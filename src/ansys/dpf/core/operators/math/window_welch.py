...
class window_welch(Operator):
    """Apply welch windowing on a given FieldsContainer having time label or a Field located on time. Assume that time sampling is evenly spaced (use time_freq_interpolation before otherwise).
      ...
      >>> op = dpf.operators.math.window_welch(field=my_field)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Apply welch windowing on a given FieldsContainer having time label or a Field located on time. Assume that time sampling is evenly spaced (use time_freq_interpolation before otherwise).""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "window::welch")
...
class InputsWindowWelch(_Inputs):
    """Intermediate class used to connect user inputs to window_welch operator
...
class OutputsWindowWelch(_Outputs):
    """Intermediate class used to get outputs from window_welch operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self.field_as_field = Output( _modify_output_spec_with_one_type(window_welch._spec().output_pin(0), "field"), 0, op) 
        self._outputs.append(self.field_as_field)
        self.field_as_fields_container = Output( _modify_output_spec_with_one_type(window_welch._spec().output_pin(0), "fields_container"), 0, op) 
