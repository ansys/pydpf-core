...
class window_hamming_fc(Operator):
    """Apply hamming windowing on a given FieldsContainer having time label or a Field located on time. Assume that time sampling is evenly spaced (use time_freq_interpolation before otherwise).
      ...
      >>> op = dpf.operators.math.window_hamming_fc(fields_container=my_fields_container)
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "window::hamming_fc")
...
class InputsWindowHammingFc(_Inputs):
    """Intermediate class used to connect user inputs to window_hamming_fc operator
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Input(window_hamming_fc._spec().input_pin(0), 0, op, -1) 
...
class OutputsWindowHammingFc(_Outputs):
    """Intermediate class used to get outputs from window_hamming_fc operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
