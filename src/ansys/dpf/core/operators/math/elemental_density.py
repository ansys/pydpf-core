...
class elemental_density(Operator):
    """Extract Elemental Topology Density result from topo solver output. Default behavior is to use graphical density.
      ...
      >>> op = dpf.operators.math.elemental_density(time_scoping=my_time_scoping,mesh_scoping=my_mesh_scoping,streams=my_streams,data_sources=my_data_sources,custom_ponderation_name=my_custom_ponderation_name)
    ...
    def __init__(self, time_scoping=None, mesh_scoping=None, streams=None, data_sources=None, custom_ponderation_name=None, config=None, server=None):
        super().__init__(name="hdf5::topo::elemental_density", config = config, server = server)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Extract Elemental Topology Density result from topo solver output. Default behavior is to use graphical density.""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "hdf5::topo::elemental_density")
...
#scripting name: elemental_density
class InputsElementalDensity(_Inputs):
    """Intermediate class used to connect user inputs to elemental_density operator
    ...
    def __init__(self, op: Operator):
        ...
        self._custom_ponderation_name = Input(elemental_density._spec().input_pin(200), 200, op, -1) 
...
class OutputsElementalDensity(_Outputs):
    """Intermediate class used to get outputs from elemental_density operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
