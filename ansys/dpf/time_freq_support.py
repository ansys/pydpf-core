...
#                txt+='\n'
    ...
    def _get_frequencies(self, cplx=False):
        ...
        if cplx == True and list_response.freq_complex.id!=0:
            return dpf.Field(channel=self._channel, field=list_response.freq_complex)
        elif list_response.freq_real.id!=0:
             return dpf.Field(channel=self._channel, field=list_response.freq_real)
        return None
    ...
    def _get_rpms(self):
        ...
        if list_response.rpm.id!=0:
            return dpf.Field(channel=self._channel, field=list_response.rpm)
        return None
    ...
    def _get_harmonic_indeces(self):
        ...
        if list_response.cyc_harmonic_index.id!=0:
            return dpf.Field(channel=self._channel, field=list_response.cyc_harmonic_index)
        return None
