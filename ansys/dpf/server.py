...
from ansys.dpf.ipconfig import ip,port
...
def _global_channel():
    ...
    if dpf.CHANNEL is None:
        raise ValueError('Please start the dpf server with dpf.start_local_instance or dpf.start_instance_using_service_manager' +
                         ' or set the Global DPF channel with dpf.CHANNEL =')
    return dpf.CHANNEL
...
def start_server_using_service_manager():
    if dpf.module_exists("grpc_interceptor_headers") :
    ...
    dpf.CHANNEL = intercept_channel
    
    dpf._server_instances.append(DpfJob(service_manager_url, dpf_service_name))
...
def start_local_server(ip=ip, port=port, dpf_path=None,
    ...
    if os.name == 'posix':
        ...
        if ansys_path is None:
            ...
            if os.name == 'posix':
                err_str += 'For example: ansys_path="/ansys_inc/v211/"'
            else:
                err_str += 'For example: ansys_path="C:\\Program Files\\ANSYS INC\v211\\'
    ...
    dpf._server_instances.append(DpfServer(dpf_path, ip, port, store_as_global=True))
...
class DpfServer:
    ...
    ansys_path : str, optional
        Path containing ansys.  For example ``/ansys_inc/v211/``
    """
