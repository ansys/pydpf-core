...
types.__doc__ = __write_enum_doc__(types,
...
def _common_progress_bar(text, unit, tot_size=None):
    if tot_size:
        widgets = [progressbar.FormatLabel(f'{text}: %(value)d of %(max_value)d {unit} '), progressbar.Percentage(),
                   progressbar.Bar()]
