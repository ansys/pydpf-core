...
class identical_meshes(Operator):
    """Take two meshes and compare them. Note: When comparing mesh properties the current behaviour is to verify that the properties in the first mesh (pin 0) are included in the second mesh (pin 1).
      ...
      >>> op = dpf.operators.logic.identical_meshes(meshA=my_meshA,meshB=my_meshB,small_value=my_small_value,tolerance=my_tolerance,compare_auxiliary=my_compare_auxiliary)

      >>> # Get output data
      >>> result_are_identical = op.outputs.are_identical()"""
    def __init__(self, meshA=None, meshB=None, small_value=None, tolerance=None, compare_auxiliary=None, config=None, server=None):
        ...
        if compare_auxiliary !=None:
            self.inputs.compare_auxiliary.connect(compare_auxiliary)

    @staticmethod
    def _spec():
        spec = Specification(description="""Take two meshes and compare them. Note: When comparing mesh properties the current behaviour is to verify that the properties in the first mesh (pin 0) are included in the second mesh (pin 1).""",
                             map_input_pin_spec={
                                 ...
                                 4 : PinSpecification(name = "compare_auxiliary", type_names=["bool"], optional=False, document="""compare auxiliary data (i.e property fields, scopings...). Default value is 'false'.""")},
                             map_output_pin_spec={
...
class InputsIdenticalMeshes(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._compare_auxiliary = Input(identical_meshes._spec().input_pin(4), 4, op, -1) 
    ...
    def compare_auxiliary(self):
        ...
        - pindoc: compare auxiliary data (i.e property fields, scopings...). Default value is 'false'.
