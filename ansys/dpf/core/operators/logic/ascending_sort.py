...
class ascending_sort(Operator):
    """Sort a field (in 0) in ascending order, with an optional component priority table or a boolean to enable sort by scoping (in 1). This operator doesn't support multiple elementary data per entity.
      ...
      >>> op = dpf.operators.logic.ascending_sort(field=my_field,component_priority_table=my_component_priority_table,sort_by_scoping=my_sort_by_scoping)
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "ascending_sort")
...
class InputsAscendingSort(_Inputs):
    """Intermediate class used to connect user inputs to ascending_sort operator
    ...
    def __init__(self, op: Operator):
        ...
        self._component_priority_table = Input(ascending_sort._spec().input_pin(1), 1, op, -1) 
...
class OutputsAscendingSort(_Outputs):
    """Intermediate class used to get outputs from ascending_sort operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
