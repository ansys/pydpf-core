...
class descending_sort_fc(Operator):
    """Sort a field (in 0) in descending order, with an optional component priority table or a boolean to enable sort by scoping (in 1). This operator doesn't support multiple elementary data per entity.
      ...
      >>> op = dpf.operators.logic.descending_sort_fc(fields_container=my_fields_container,component_priority_table=my_component_priority_table,sort_by_scoping=my_sort_by_scoping)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Sort a field (in 0) in descending order, with an optional component priority table or a boolean to enable sort by scoping (in 1). This operator doesn't support multiple elementary data per entity.""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "descending_sort_fc")
...
class InputsDescendingSortFc(_Inputs):
    """Intermediate class used to connect user inputs to descending_sort_fc operator
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Input(descending_sort_fc._spec().input_pin(0), 0, op, -1) 
        self._inputs.append(self._fields_container)
        self._component_priority_table = Input(descending_sort_fc._spec().input_pin(1), 1, op, -1) 
        self._inputs.append(self._component_priority_table)
        self._sort_by_scoping = Input(descending_sort_fc._spec().input_pin(2), 2, op, -1) 
...
class OutputsDescendingSortFc(_Outputs):
    """Intermediate class used to get outputs from descending_sort_fc operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
