...
class data_tree_to_txt(Operator):
    """Writes a txt file or string from a DataTree
      ...
      >>> op = dpf.operators.serialization.data_tree_to_txt(data_tree=my_data_tree,path=my_path)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Writes a txt file or string from a DataTree""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "data_tree_to_txt")
...
class InputsDataTreeToTxt(_Inputs):
    """Intermediate class used to connect user inputs to data_tree_to_txt operator
...
class OutputsDataTreeToTxt(_Outputs):
    """Intermediate class used to get outputs from data_tree_to_txt operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self.data_sources_as_data_sources = Output( _modify_output_spec_with_one_type(data_tree_to_txt._spec().output_pin(0), "data_sources"), 0, op) 
        self._outputs.append(self.data_sources_as_data_sources)
        self.data_sources_as_string = Output( _modify_output_spec_with_one_type(data_tree_to_txt._spec().output_pin(0), "string"), 0, op) 
