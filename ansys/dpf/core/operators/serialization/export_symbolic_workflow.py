...
class export_symbolic_workflow(Operator):
    """Transforms a Workflow into a symbolic Workflow and writes it to a file (if a path is set in input) or string
      ...
      >>> op = dpf.operators.serialization.export_symbolic_workflow(workflow=my_workflow,path=my_path,format=my_format)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Transforms a Workflow into a symbolic Workflow and writes it to a file (if a path is set in input) or string""",
                             map_input_pin_spec={
                                 0 : PinSpecification(name = "workflow", type_names=["workflow"], optional=False, document=""""""), 
                                 1 : PinSpecification(name = "path", type_names=["string"], optional=True, document=""""""), 
                                 2 : PinSpecification(name = "format", type_names=["int32"], optional=True, document="""0 is ASCII format and 1 is binary, default is 0.""")},
                             map_output_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "export_symbolic_workflow")
...
class InputsExportSymbolicWorkflow(_Inputs):
    """Intermediate class used to connect user inputs to export_symbolic_workflow operator
    ...
    def format(self):
        ...
        - pindoc: 0 is ASCII format and 1 is binary, default is 0.
...
class OutputsExportSymbolicWorkflow(_Outputs):
    """Intermediate class used to get outputs from export_symbolic_workflow operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self.data_sources_as_data_sources = Output( _modify_output_spec_with_one_type(export_symbolic_workflow._spec().output_pin(0), "data_sources"), 0, op) 
        self._outputs.append(self.data_sources_as_data_sources)
        self.data_sources_as_string = Output( _modify_output_spec_with_one_type(export_symbolic_workflow._spec().output_pin(0), "string"), 0, op) 
