...
class elemental_nodal_to_nodal(Operator):
    """Transform ElementalNodal field into Nodal field using an averaging process, result is computed on a given node scoping.
      ...
      >>> op = dpf.operators.averaging.elemental_nodal_to_nodal(field=my_field,mesh_scoping=my_mesh_scoping,should_average=my_should_average,extend_to_mid_nodes=my_extend_to_mid_nodes,mesh=my_mesh)

      >>> # Get output data
      >>> result_field = op.outputs.field()
      >>> result_weight = op.outputs.weight()"""
    def __init__(self, field=None, mesh_scoping=None, should_average=None, extend_to_mid_nodes=None, mesh=None, config=None, server=None):
    ...
    def _spec():
        spec = Specification(description="""Transform ElementalNodal field into Nodal field using an averaging process, result is computed on a given node scoping.""",
                             ...
                             map_output_pin_spec={
                                 ...
                                 1 : PinSpecification(name = "weight", type_names=["property_field"], optional=False, document="""Gives for each node, the number of times it was found in the elemental nodal field. Can be used to average later.""")})
        return spec
...
class InputsElementalNodalToNodal(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._extend_to_mid_nodes = Input(elemental_nodal_to_nodal._spec().input_pin(4), 4, op, -1) 
...
class OutputsElementalNodalToNodal(_Outputs):
    ...
    def weight(self):
        ...
        - pindoc: Gives for each node, the number of times it was found in the elemental nodal field. Can be used to average later.
