...
class elemental_nodal_to_nodal_fc(Operator):
    """Transform ElementalNodal fields into Nodal fields using an averaging process, result is computed on a given node scoping. If the input fields are mixed shell/solid, then the fields are split by element shape and the output fields container has elshape label.
      ...
      >>> op = dpf.operators.averaging.elemental_nodal_to_nodal_fc(fields_container=my_fields_container,mesh=my_mesh,should_average=my_should_average,scoping=my_scoping,extend_to_mid_nodes=my_extend_to_mid_nodes)

      >>> # Get output data
      >>> result_fields_container = op.outputs.fields_container()
      >>> result_weights = op.outputs.weights()"""
    def __init__(self, fields_container=None, mesh=None, should_average=None, scoping=None, extend_to_mid_nodes=None, config=None, server=None):
        ...
        if extend_to_mid_nodes !=None:
            self.inputs.extend_to_mid_nodes.connect(extend_to_mid_nodes)

    @staticmethod
    def _spec():
        spec = Specification(description="""Transform ElementalNodal fields into Nodal fields using an averaging process, result is computed on a given node scoping. If the input fields are mixed shell/solid, then the fields are split by element shape and the output fields container has elshape label.""",
                             map_input_pin_spec={
                                 ...
                                 4 : PinSpecification(name = "extend_to_mid_nodes", type_names=["bool"], optional=True, document="""Compute mid nodes (when available) by averaging neighbour primary nodes""")},
                             map_output_pin_spec={
                                 0 : PinSpecification(name = "fields_container", type_names=["fields_container"], optional=False, document=""""""), 
                                 1 : PinSpecification(name = "weights", type_names=[], optional=False, document="""Gives for each node, the number of times it was found in the elemental nodal field. Can be used to average later.""")})
        return spec
...
class InputsElementalNodalToNodalFc(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._extend_to_mid_nodes = Input(elemental_nodal_to_nodal_fc._spec().input_pin(4), 4, op, -1) 
    ...
    def extend_to_mid_nodes(self):
        ...
        - pindoc: Compute mid nodes (when available) by averaging neighbour primary nodes
