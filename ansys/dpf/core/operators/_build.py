...
'''
    ...
    lines = cls.split("\n")
    lines = ensure_one_trailing_blank_line(lines)
    cls = "\n".join([line.rstrip() for line in lines])

    return cls

def ensure_one_trailing_blank_line(lines):
    blank_lines = 0

    # Count blank lines
    while not lines[-(blank_lines + 1)]:
        blank_lines += 1

    # The last line wasn't blank, so add one.
    if blank_lines == 0:
        return [*lines, ""]

    # There was only one blank line, so just return.
    if blank_lines == 1:
        return lines

    # Slice list, ensuring we keep one blank line.
    return lines[:-blank_lines + 1]
