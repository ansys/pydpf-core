...
class exponential_fc(Operator):
    """Computes element-wise exp(field[i]).
      ...
      >>> op = dpf.operators.math.exponential_fc(fields_container=my_fields_container)
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "exponential_fc")
...
class InputsExponentialFc(_Inputs):
    """Intermediate class used to connect user inputs to exponential_fc operator
...
class OutputsExponentialFc(_Outputs):
    """Intermediate class used to get outputs from exponential_fc operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
