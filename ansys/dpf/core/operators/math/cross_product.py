...
class cross_product(Operator):
    """Computes the cross product of two vector fields. Fields could have the same location or Elemental-Nodal and Nodal.
      ...
      >>> op = dpf.operators.math.cross_product(fieldA=my_fieldA,fieldB=my_fieldB)
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "cross_product")
...
class InputsCrossProduct(_Inputs):
    """Intermediate class used to connect user inputs to cross_product operator
...
class OutputsCrossProduct(_Outputs):
    """Intermediate class used to get outputs from cross_product operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
