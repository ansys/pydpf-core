...
class convertnum_bcs_to_nod(Operator):
    """Converts fields container from BCS to NOD ordering
      ...
      >>> op = dpf.operators.invariant.convertnum_bcs_to_nod(fields_container=my_fields_container,data_sources=my_data_sources)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Converts fields container from BCS to NOD ordering""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "convertnum_bcs_to_nod")
...
class InputsConvertnumBcsToNod(_Inputs):
    """Intermediate class used to connect user inputs to convertnum_bcs_to_nod operator
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Input(convertnum_bcs_to_nod._spec().input_pin(2), 2, op, -1) 
        self._inputs.append(self._fields_container)
        self._data_sources = Input(convertnum_bcs_to_nod._spec().input_pin(4), 4, op, -1) 
...
class OutputsConvertnumBcsToNod(_Outputs):
    """Intermediate class used to get outputs from convertnum_bcs_to_nod operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Output(convertnum_bcs_to_nod._spec().output_pin(0), 0, op) 
