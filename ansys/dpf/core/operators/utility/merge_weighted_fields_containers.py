...
class merge_weighted_fields_containers(Operator):
    """Take a set of fields containers and assemble them in a unique one applying a weight on the sum of the fields.
      ...
      >>> op = dpf.operators.utility.merge_weighted_fields_containers(sum_merge=my_sum_merge,merged_fields_support=my_merged_fields_support,merged_fields_containers_support=my_merged_fields_containers_support,fields_containers1=my_fields_containers1,fields_containers2=my_fields_containers2,weights1=my_weights1,weights2=my_weights2)

      >>> # Get output data
      >>> result_merged_fields_container = op.outputs.merged_fields_container()"""
    def __init__(self, sum_merge=None, merged_fields_support=None, merged_fields_containers_support=None, fields_containers1=None, fields_containers2=None, weights1=None, weights2=None, config=None, server=None):
        super().__init__(name="merge::weighted_fields_container", config = config, server = server)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Take a set of fields containers and assemble them in a unique one applying a weight on the sum of the fields.""",
                             map_input_pin_spec={
                                 ...
                                 1000 : PinSpecification(name = "weights", type_names=["property_field"], optional=False, document="""Weights to apply to each field from pin 1000 to ..."""), 
                                 1001 : PinSpecification(name = "weights", type_names=["property_field"], optional=False, document="""Weights to apply to each field from pin 1000 to ...""")},
                             map_output_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "merge::weighted_fields_container")
...
class InputsMergeWeightedFieldsContainers(_Inputs):
    """Intermediate class used to connect user inputs to merge_weighted_fields_containers operator
    ...
    def __init__(self, op: Operator):
        ...
        self._sum_merge = Input(merge_weighted_fields_containers._spec().input_pin(-3), -3, op, -1) 
        self._inputs.append(self._sum_merge)
        self._merged_fields_support = Input(merge_weighted_fields_containers._spec().input_pin(-2), -2, op, -1) 
        self._inputs.append(self._merged_fields_support)
        self._merged_fields_containers_support = Input(merge_weighted_fields_containers._spec().input_pin(-1), -1, op, -1) 
        self._inputs.append(self._merged_fields_containers_support)
        self._fields_containers1 = Input(merge_weighted_fields_containers._spec().input_pin(0), 0, op, 0) 
        self._inputs.append(self._fields_containers1)
        self._fields_containers2 = Input(merge_weighted_fields_containers._spec().input_pin(1), 1, op, 1) 
        self._inputs.append(self._fields_containers2)
        self._weights1 = Input(merge_weighted_fields_containers._spec().input_pin(1000), 1000, op, 0) 
        self._inputs.append(self._weights1)
        self._weights2 = Input(merge_weighted_fields_containers._spec().input_pin(1001), 1001, op, 1) 
    ...
    def weights1(self):
        ...
        - pindoc: Weights to apply to each field from pin 1000 to ...
...
class OutputsMergeWeightedFieldsContainers(_Outputs):
    """Intermediate class used to get outputs from merge_weighted_fields_containers operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self._merged_fields_container = Output(merge_weighted_fields_containers._spec().output_pin(0), 0, op) 
