...
class extract_scoping(Operator):
    """Take a field or a fields container and extract is scoping or scopings container.

      available inputs:
        - field_or_fields_container (Field, FieldsContainer) (optional)
      ...
      >>> op = dpf.operators.utility.extract_scoping(field_or_fields_container=my_field_or_fields_container)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Take a field or a fields container and extract is scoping or scopings container.""",
                             map_input_pin_spec={
                                 0 : PinSpecification(name = "field_or_fields_container", type_names=["field","fields_container"], optional=True, document="""""")},
                             map_output_pin_spec={
                                 0 : PinSpecification(name = "mesh_scoping", type_names=["scoping","scopings_container"], optional=False, document="""""")})
        return spec


    @staticmethod
    def default_config():
        return Operator.default_config(name = "extract_scoping")
...
class InputsExtractScoping(_Inputs):
    """Intermediate class used to connect user inputs to extract_scoping operator
    ...
    def __init__(self, op: Operator):
        ...
        self._field_or_fields_container = Input(extract_scoping._spec().input_pin(0), 0, op, -1) 
...
class OutputsExtractScoping(_Outputs):
    """Intermediate class used to get outputs from extract_scoping operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self.mesh_scoping_as_scoping = Output( _modify_output_spec_with_one_type(extract_scoping._spec().output_pin(0), "scoping"), 0, op) 
        self._outputs.append(self.mesh_scoping_as_scoping)
        self.mesh_scoping_as_scopings_container = Output( _modify_output_spec_with_one_type(extract_scoping._spec().output_pin(0), "scopings_container"), 0, op) 
