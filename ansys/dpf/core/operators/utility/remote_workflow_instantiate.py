...
class remote_workflow_instantiate(Operator):
    """Sends a local workflow to a remote process (and keep a local image of it) or create a local image of an existing remote workflow (identified by an id and an address) for a given protocol registered in the streams.
      ...
      >>> op = dpf.operators.utility.remote_workflow_instantiate(workflow_to_send=my_workflow_to_send,streams_to_remote=my_streams_to_remote,data_sources_to_remote=my_data_sources_to_remote)

      >>> # Get output data
      >>> result_remote_workflow = op.outputs.remote_workflow()"""
    def __init__(self, workflow_to_send=None, streams_to_remote=None, data_sources_to_remote=None, config=None, server=None):
        super().__init__(name="remote_workflow_instantiate", config = config, server = server)
        self._inputs = InputsRemoteWorkflowInstantiate(self)
        self._outputs = OutputsRemoteWorkflowInstantiate(self)
        if workflow_to_send !=None:
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Sends a local workflow to a remote process (and keep a local image of it) or create a local image of an existing remote workflow (identified by an id and an address) for a given protocol registered in the streams.""",
                             map_input_pin_spec={
                                 0 : PinSpecification(name = "workflow_to_send", type_names=["workflow","int32"], optional=False, document="""local workflow to push to a remote or id of a remote workflow"""), 
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "remote_workflow_instantiate")
...
class InputsRemoteWorkflowInstantiate(_Inputs):
    """Intermediate class used to connect user inputs to remote_workflow_instantiate operator
    ...
    def __init__(self, op: Operator):
        ...
        self._workflow_to_send = Input(remote_workflow_instantiate._spec().input_pin(0), 0, op, -1) 
        self._inputs.append(self._workflow_to_send)
        self._streams_to_remote = Input(remote_workflow_instantiate._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._streams_to_remote)
        self._data_sources_to_remote = Input(remote_workflow_instantiate._spec().input_pin(4), 4, op, -1) 
    ...
    def workflow_to_send(self):
        ...
        Parameters
        ----------
        my_workflow_to_send : Workflow, int, 
...
class OutputsRemoteWorkflowInstantiate(_Outputs):
    """Intermediate class used to get outputs from remote_workflow_instantiate operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self._remote_workflow = Output(remote_workflow_instantiate._spec().output_pin(0), 0, op) 
