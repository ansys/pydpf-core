...
class assemble_scalars_to_vectors(Operator):
    """Take three scalar fields and assemble them as a 3d vector field.
      ...
      >>> op = dpf.operators.utility.assemble_scalars_to_vectors(x=my_x,y=my_y,z=my_z)

      >>> # Get output data
      >>> result_field = op.outputs.field()"""
    def __init__(self, x=None, y=None, z=None, config=None, server=None):
        super().__init__(name="assemble_scalars_to_vectors", config = config, server = server)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Take three scalar fields and assemble them as a 3d vector field.""",
                             map_input_pin_spec={
                                 0 : PinSpecification(name = "x", type_names=["field"], optional=True, document=""""""), 
                                 1 : PinSpecification(name = "y", type_names=["field"], optional=True, document=""""""), 
                                 2 : PinSpecification(name = "z", type_names=["field"], optional=True, document="""""")},
                             map_output_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "assemble_scalars_to_vectors")
...
class InputsAssembleScalarsToVectors(_Inputs):
    """Intermediate class used to connect user inputs to assemble_scalars_to_vectors operator
...
class OutputsAssembleScalarsToVectors(_Outputs):
    """Intermediate class used to get outputs from assemble_scalars_to_vectors operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
