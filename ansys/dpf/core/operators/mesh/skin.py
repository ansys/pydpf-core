    ...
    def _spec():
        spec = Specification(description="""Extracts a skin of the mesh (2D elements) in a new meshed region. Material id of initial elements are propagated to their facets.""",
                             ...
                             map_output_pin_spec={
                                 ...
                                 3 : PinSpecification(name = "property_field_new_elements_to_old", type_names=["property_field"], optional=False, document="""This Property field gives, for each new face element id (in the scoping) the corresponding 3D volume element index (in the data) it has been extracted from. The 3D volume element id can be found with the element scoping of the input mesh."""), 
                                 4 : PinSpecification(name = "facet_indices", type_names=["property_field"], optional=False, document="""This Property field fives, for each new face element id (in the scoping) the corresponding face index on the source 3D volume element. The 3d volume element can be extracted from the previous output.""")})
        return spec
