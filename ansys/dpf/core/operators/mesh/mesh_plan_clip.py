...
class mesh_plan_clip(Operator):
    """Clip a volume mesh along a plane, and keep one side.
      ...
      available inputs:
        - mesh_or_field (MeshedRegion, Field)
        - normal (Field)
        - origin (Field)
      ...
      >>> op = dpf.operators.mesh.mesh_plan_clip(mesh_or_field=my_mesh_or_field,normal=my_normal,origin=my_origin)

      >>> # Get output data
      >>> result_field = op.outputs.field()
      >>> result_mesh = op.outputs.mesh()"""
    def __init__(self, mesh_or_field=None, normal=None, origin=None, config=None, server=None):
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Clip a volume mesh along a plane, and keep one side.""",
                             map_input_pin_spec={
                                 0 : PinSpecification(name = "mesh_or_field", type_names=["abstract_meshed_region","field"], optional=False, document=""""""), 
                             ...
                             map_output_pin_spec={
                                 ...
                                 2 : PinSpecification(name = "mesh", type_names=["abstract_meshed_region"], optional=False, document="""""")})
        return spec


    @staticmethod
    def default_config():
        return Operator.default_config(name = "mesh_plan_clip")
...
class InputsMeshPlanClip(_Inputs):
    """Intermediate class used to connect user inputs to mesh_plan_clip operator
    ...
    def mesh_or_field(self):
        ...
        Parameters
        ----------
        my_mesh_or_field : MeshedRegion, Field, 
...
class OutputsMeshPlanClip(_Outputs):
    """Intermediate class used to get outputs from mesh_plan_clip operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
