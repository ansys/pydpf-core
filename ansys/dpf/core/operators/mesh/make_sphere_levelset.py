...
class make_sphere_levelset(Operator):
    """Compute the levelset for a sphere using coordinates.
      ...
      >>> op = dpf.operators.mesh.make_sphere_levelset(coordinates=my_coordinates,origin=my_origin,radius=my_radius)

      >>> # Get output data
      >>> result_field = op.outputs.field()"""
    def __init__(self, coordinates=None, origin=None, radius=None, config=None, server=None):
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Compute the levelset for a sphere using coordinates.""",
                             map_input_pin_spec={
                                 0 : PinSpecification(name = "coordinates", type_names=["abstract_meshed_region","field"], optional=False, document=""""""), 
                                 1 : PinSpecification(name = "origin", type_names=["field"], optional=False, document="""An overall 3d vector that gives a point of the plane."""), 
                                 2 : PinSpecification(name = "radius", type_names=["double"], optional=False, document="""Sphere radius.""")},
                             map_output_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "levelset::make_sphere")
...
class InputsMakeSphereLevelset(_Inputs):
    """Intermediate class used to connect user inputs to make_sphere_levelset operator
...
class OutputsMakeSphereLevelset(_Outputs):
    """Intermediate class used to get outputs from make_sphere_levelset operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
