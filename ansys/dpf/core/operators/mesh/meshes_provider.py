...
class meshes_provider(Operator):
    """Read meshes from result files. Meshes can be spatially or temporally varying.
      ...
      >>> op = dpf.operators.mesh.meshes_provider(time_scoping=my_time_scoping,streams_container=my_streams_container,data_sources=my_data_sources)

      >>> # Get output data
      >>> result_meshes = op.outputs.meshes()"""
    def __init__(self, time_scoping=None, streams_container=None, data_sources=None, config=None, server=None):
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Read meshes from result files. Meshes can be spatially or temporally varying.""",
                             map_input_pin_spec={
                                 0 : PinSpecification(name = "time_scoping", type_names=["scoping","vector<int32>","int32"], optional=True, document="""time/freq set ids required in output"""), 
                             ...
                             map_output_pin_spec={
                                 0 : PinSpecification(name = "meshes", type_names=["meshes_container"], optional=False, document="""""")})
        return spec


    @staticmethod
    def default_config():
        return Operator.default_config(name = "meshes_provider")
...
class InputsMeshesProvider(_Inputs):
    """Intermediate class used to connect user inputs to meshes_provider operator
    ...
    def time_scoping(self):
        ...
        - pindoc: time/freq set ids required in output
...
class OutputsMeshesProvider(_Outputs):
    """Intermediate class used to get outputs from meshes_provider operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
