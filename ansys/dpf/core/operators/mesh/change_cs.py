...
class change_cs(Operator):
    """Apply a transformation (rotation and displacement) matrix on a mesh or meshes container.
      ...
      available inputs:
        - meshes (MeshedRegion, MeshesContainer)
      ...
      >>> op = dpf.operators.mesh.change_cs(meshes=my_meshes,coordinate_system=my_coordinate_system)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Apply a transformation (rotation and displacement) matrix on a mesh or meshes container.""",
                             map_input_pin_spec={
                                 0 : PinSpecification(name = "meshes", type_names=["meshed_region","meshes_container"], optional=False, document=""""""), 
                                 1 : PinSpecification(name = "coordinate_system", type_names=["field"], optional=False, document="""3-3 rotation matrix + 3 translations (X, Y, Z)""")},
                             map_output_pin_spec={
                                 0 : PinSpecification(name = "meshed_region", type_names=["meshed_region","meshes_container"], optional=False, document="""""")})
        return spec


    @staticmethod
    def default_config():
        return Operator.default_config(name = "mesh::change_cs")
...
class InputsChangeCs(_Inputs):
    """Intermediate class used to connect user inputs to change_cs operator
    ...
    def meshes(self):
        ...
        Parameters
        ----------
        my_meshes : MeshedRegion, MeshesContainer, 
    ...
    def coordinate_system(self):
        ...
        - pindoc: 3-3 rotation matrix + 3 translations (X, Y, Z)
...
class OutputsChangeCs(_Outputs):
    """Intermediate class used to get outputs from change_cs operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self.meshed_region_as_meshed_region = Output( _modify_output_spec_with_one_type(change_cs._spec().output_pin(0), "meshed_region"), 0, op) 
        self._outputs.append(self.meshed_region_as_meshed_region)
        self.meshed_region_as_meshes_container = Output( _modify_output_spec_with_one_type(change_cs._spec().output_pin(0), "meshes_container"), 0, op) 
