...
class find_reduced_coordinates(Operator):
    """Find the elements corresponding to the given coordinates in input and compute their reduced coordinates in those elements.
      ...
      >>> op = dpf.operators.mapping.find_reduced_coordinates(coordinates=my_coordinates,mesh=my_mesh,use_quadratic_elements=my_use_quadratic_elements)

      >>> # Get output data
      >>> result_reduced_coordinates = op.outputs.reduced_coordinates()
      >>> result_element_ids = op.outputs.element_ids()"""
    def __init__(self, coordinates=None, mesh=None, use_quadratic_elements=None, config=None, server=None):
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Find the elements corresponding to the given coordinates in input and compute their reduced coordinates in those elements.""",
                             map_input_pin_spec={
                                 ...
                                 200 : PinSpecification(name = "use_quadratic_elements", type_names=["bool"], optional=False, document="""If this pin is set to true reduced coordinates are computed on the quadratic element if the element is quadratic (default is false)""")},
                             map_output_pin_spec={
                                 0 : PinSpecification(name = "reduced_coordinates", type_names=["fields_container"], optional=False, document="""coordinates in the reference elements"""), 
                                 1 : PinSpecification(name = "element_ids", type_names=["scopings_container"], optional=False, document="""Ids of the elements where each set of reduced coordinates is found""")})
        return spec


    @staticmethod
    def default_config():
        return Operator.default_config(name = "find_reduced_coordinates")
...
class InputsFindReducedCoordinates(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._coordinates = Input(find_reduced_coordinates._spec().input_pin(1), 1, op, -1) 
        self._inputs.append(self._coordinates)
        self._mesh = Input(find_reduced_coordinates._spec().input_pin(7), 7, op, -1) 
        self._inputs.append(self._mesh)
        self._use_quadratic_elements = Input(find_reduced_coordinates._spec().input_pin(200), 200, op, -1) 
    ...
    def use_quadratic_elements(self):
        ...
        - pindoc: If this pin is set to true reduced coordinates are computed on the quadratic element if the element is quadratic (default is false)
...
class OutputsFindReducedCoordinates(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._element_ids = Output(find_reduced_coordinates._spec().output_pin(1), 1, op) 
    ...
    def reduced_coordinates(self):
        ...
        - pindoc: coordinates in the reference elements
    ...
    def element_ids(self):
        ...
        - pindoc: Ids of the elements where each set of reduced coordinates is found
