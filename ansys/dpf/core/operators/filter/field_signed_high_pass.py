...
class field_signed_high_pass(Operator):
    """The high pass filter returns all the values superior or equal in absolute value to the threshold value in input.
      ...
      >>> op = dpf.operators.filter.field_signed_high_pass(field=my_field,threshold=my_threshold,both=my_both)

      >>> # Get output data
      >>> result_field = op.outputs.field()"""
    def __init__(self, field=None, threshold=None, both=None, config=None, server=None):
        super().__init__(name="core::field::signed_high_pass", config = config, server = server)
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "core::field::signed_high_pass")
...
class InputsFieldSignedHighPass(_Inputs):
    """Intermediate class used to connect user inputs to field_signed_high_pass operator
...
class OutputsFieldSignedHighPass(_Outputs):
    """Intermediate class used to get outputs from field_signed_high_pass operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
