...
All units need to be consistent.Get the YZ shear component (12 component).
      ...
      >>> op = dpf.operators.result.compute_stress_YZ(scoping=my_scoping,streams_container=my_streams_container,data_sources=my_data_sources,requested_location=my_requested_location,strain=my_strain)
...
class InputsComputeStressYZ(_Inputs):
    """Intermediate class used to connect user inputs to compute_stress_YZ operator
    ...
    def __init__(self, op: Operator):
        ...
        self._streams_container = Input(compute_stress_YZ._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._streams_container)
        self._data_sources = Input(compute_stress_YZ._spec().input_pin(4), 4, op, -1) 
        self._inputs.append(self._data_sources)
        self._requested_location = Input(compute_stress_YZ._spec().input_pin(9), 9, op, -1) 
...
class OutputsComputeStressYZ(_Outputs):
    """Intermediate class used to get outputs from compute_stress_YZ operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
