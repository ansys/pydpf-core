...
class cyclic_expanded_heat_flux(Operator):
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Read mapdl::rth::TF from an rst file and expand it with cyclic symmetry.""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "mapdl::rth::TF_cyclic")
...
class InputsCyclicExpandedHeatFlux(_Inputs):
    """Intermediate class used to connect user inputs to cyclic_expanded_heat_flux operator
    ...
    def __init__(self, op: Operator):
        ...
        self._time_scoping = Input(cyclic_expanded_heat_flux._spec().input_pin(0), 0, op, -1) 
        self._inputs.append(self._time_scoping)
        self._mesh_scoping = Input(cyclic_expanded_heat_flux._spec().input_pin(1), 1, op, -1) 
        self._inputs.append(self._mesh_scoping)
        self._fields_container = Input(cyclic_expanded_heat_flux._spec().input_pin(2), 2, op, -1) 
        self._inputs.append(self._fields_container)
        self._streams_container = Input(cyclic_expanded_heat_flux._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._streams_container)
        self._data_sources = Input(cyclic_expanded_heat_flux._spec().input_pin(4), 4, op, -1) 
        self._inputs.append(self._data_sources)
        self._bool_rotate_to_global = Input(cyclic_expanded_heat_flux._spec().input_pin(5), 5, op, -1) 
        self._inputs.append(self._bool_rotate_to_global)
        self._sector_mesh = Input(cyclic_expanded_heat_flux._spec().input_pin(7), 7, op, -1) 
        self._inputs.append(self._sector_mesh)
        self._requested_location = Input(cyclic_expanded_heat_flux._spec().input_pin(9), 9, op, -1) 
        self._inputs.append(self._requested_location)
        self._read_cyclic = Input(cyclic_expanded_heat_flux._spec().input_pin(14), 14, op, -1) 
        self._inputs.append(self._read_cyclic)
        self._expanded_meshed_region = Input(cyclic_expanded_heat_flux._spec().input_pin(15), 15, op, -1) 
        self._inputs.append(self._expanded_meshed_region)
        self._cyclic_support = Input(cyclic_expanded_heat_flux._spec().input_pin(16), 16, op, -1) 
        self._inputs.append(self._cyclic_support)
        self._sectors_to_expand = Input(cyclic_expanded_heat_flux._spec().input_pin(18), 18, op, -1) 
...
class OutputsCyclicExpandedHeatFlux(_Outputs):
    """Intermediate class used to get outputs from cyclic_expanded_heat_flux operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Output(cyclic_expanded_heat_flux._spec().output_pin(0), 0, op) 
        self._outputs.append(self._fields_container)
        self._expanded_meshes = Output(cyclic_expanded_heat_flux._spec().output_pin(1), 1, op) 
