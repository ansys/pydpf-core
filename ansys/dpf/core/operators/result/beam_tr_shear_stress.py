...
class beam_tr_shear_stress(Operator):
    """Read Beam TR Shear Stress (LSDyna) by calling the readers defined by the datasources.
      ...
      >>> op = dpf.operators.result.beam_tr_shear_stress(time_scoping=my_time_scoping,mesh_scoping=my_mesh_scoping,data_sources=my_data_sources,integration_point=my_integration_point)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Read Beam TR Shear Stress (LSDyna) by calling the readers defined by the datasources.""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "B_ST2")
...
class InputsBeamTrShearStress(_Inputs):
    """Intermediate class used to connect user inputs to beam_tr_shear_stress operator
    ...
    def __init__(self, op: Operator):
        ...
        self._streams_container = Input(beam_tr_shear_stress._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._streams_container)
        self._data_sources = Input(beam_tr_shear_stress._spec().input_pin(4), 4, op, -1) 
        self._inputs.append(self._data_sources)
        self._integration_point = Input(beam_tr_shear_stress._spec().input_pin(6), 6, op, -1) 
...
class OutputsBeamTrShearStress(_Outputs):
    """Intermediate class used to get outputs from beam_tr_shear_stress operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Output(beam_tr_shear_stress._spec().output_pin(0), 0, op) 
