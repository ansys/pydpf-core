...
class interface_contact_moment(Operator):
    """Read Interface Contact Moment (LSDyna) by calling the readers defined by the datasources.
      ...
      >>> op = dpf.operators.result.interface_contact_moment(data_sources=my_data_sources,entity_scoping=my_entity_scoping)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Read Interface Contact Moment (LSDyna) by calling the readers defined by the datasources.""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "R_CM")
...
class InputsInterfaceContactMoment(_Inputs):
    """Intermediate class used to connect user inputs to interface_contact_moment operator
    ...
    def __init__(self, op: Operator):
        ...
        self._streams_container = Input(interface_contact_moment._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._streams_container)
        self._data_sources = Input(interface_contact_moment._spec().input_pin(4), 4, op, -1) 
        self._inputs.append(self._data_sources)
        self._entity_scoping = Input(interface_contact_moment._spec().input_pin(6), 6, op, -1) 
...
class OutputsInterfaceContactMoment(_Outputs):
    """Intermediate class used to get outputs from interface_contact_moment operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Output(interface_contact_moment._spec().output_pin(0), 0, op) 
