...
class compute_stress_XY_XY(Operator):
...
All units need to be consistent.Get the XY shear component (01 component).Get the XY shear component (01 component).
      ...
      >>> op = dpf.operators.result.compute_stress_XY_XY()
      ...
      >>> op = dpf.operators.result.compute_stress_XY_XY(scoping=my_scoping,streams_container=my_streams_container,data_sources=my_data_sources,requested_location=my_requested_location,strain=my_strain)
    ...
    def __init__(self, scoping=None, streams_container=None, data_sources=None, requested_location=None, strain=None, config=None, server=None):
        ...
        self._inputs = InputsComputeStressXYXY(self)
        self._outputs = OutputsComputeStressXYXY(self)
        if scoping !=None:
...
All units need to be consistent.Get the XY shear component (01 component).Get the XY shear component (01 component).""",
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "compute_stress_XY")
    ...
    def inputs(self):
        ...
        Returns
        --------
        inputs : InputsComputeStressXYXY 
        """
        return super().inputs
    ...
    def outputs(self):
        ...
        Returns
        --------
        outputs : OutputsComputeStressXYXY 
        """
        return super().outputs


#internal name: compute_stress_XY
#scripting name: compute_stress_XY_XY
class InputsComputeStressXYXY(_Inputs):
    """Intermediate class used to connect user inputs to compute_stress_XY_XY operator
    ...
    def __init__(self, op: Operator):
        super().__init__(compute_stress_XY_XY._spec().inputs, op)
        self._scoping = Input(compute_stress_XY_XY._spec().input_pin(1), 1, op, -1) 
        self._inputs.append(self._scoping)
        self._streams_container = Input(compute_stress_XY_XY._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._streams_container)
        self._data_sources = Input(compute_stress_XY_XY._spec().input_pin(4), 4, op, -1) 
        self._inputs.append(self._data_sources)
        self._requested_location = Input(compute_stress_XY_XY._spec().input_pin(9), 9, op, -1) 
        self._inputs.append(self._requested_location)
        self._strain = Input(compute_stress_XY_XY._spec().input_pin(10), 10, op, -1) 
...
class OutputsComputeStressXYXY(_Outputs):
    """Intermediate class used to get outputs from compute_stress_XY_XY operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    """
    def __init__(self, op: Operator):
        super().__init__(compute_stress_XY_XY._spec().outputs, op)
        self._fields_container = Output(compute_stress_XY_XY._spec().output_pin(0), 0, op) 
