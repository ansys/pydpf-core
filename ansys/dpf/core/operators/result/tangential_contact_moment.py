...
class tangential_contact_moment(Operator):
    """Read/compute tangential contact moment by calling the readers defined by the datasources.
      ...
      >>> op = dpf.operators.result.tangential_contact_moment(time_scoping=my_time_scoping,mesh_scoping=my_mesh_scoping,data_sources=my_data_sources)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Read/compute tangential contact moment by calling the readers defined by the datasources.""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "CFTTAN")
...
class InputsTangentialContactMoment(_Inputs):
    """Intermediate class used to connect user inputs to tangential_contact_moment operator
    ...
    def __init__(self, op: Operator):
        ...
        self._time_scoping = Input(tangential_contact_moment._spec().input_pin(0), 0, op, -1) 
        self._inputs.append(self._time_scoping)
        self._mesh_scoping = Input(tangential_contact_moment._spec().input_pin(1), 1, op, -1) 
        self._inputs.append(self._mesh_scoping)
        self._fields_container = Input(tangential_contact_moment._spec().input_pin(2), 2, op, -1) 
        self._inputs.append(self._fields_container)
        self._streams_container = Input(tangential_contact_moment._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._streams_container)
        self._data_sources = Input(tangential_contact_moment._spec().input_pin(4), 4, op, -1) 
        self._inputs.append(self._data_sources)
        self._bool_rotate_to_global = Input(tangential_contact_moment._spec().input_pin(5), 5, op, -1) 
        self._inputs.append(self._bool_rotate_to_global)
        self._mesh = Input(tangential_contact_moment._spec().input_pin(7), 7, op, -1) 
        self._inputs.append(self._mesh)
        self._read_cyclic = Input(tangential_contact_moment._spec().input_pin(14), 14, op, -1) 
...
class OutputsTangentialContactMoment(_Outputs):
    """Intermediate class used to get outputs from tangential_contact_moment operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Output(tangential_contact_moment._spec().output_pin(0), 0, op) 
