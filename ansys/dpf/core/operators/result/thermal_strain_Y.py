...
class thermal_strain_Y(Operator):
    """Read/compute element nodal component thermal strains YY normal component (11 component) by calling the readers defined by the datasources. Regarding the requested location and the input mesh scoping, the result location can be Nodal/ElementalNodal/Elemental.
      ...
      >>> op = dpf.operators.result.thermal_strain_Y(time_scoping=my_time_scoping,mesh_scoping=my_mesh_scoping,data_sources=my_data_sources,requested_location=my_requested_location)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Read/compute element nodal component thermal strains YY normal component (11 component) by calling the readers defined by the datasources. Regarding the requested location and the input mesh scoping, the result location can be Nodal/ElementalNodal/Elemental.""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "ETHY")
...
class InputsThermalStrainY(_Inputs):
    """Intermediate class used to connect user inputs to thermal_strain_Y operator
    ...
    def __init__(self, op: Operator):
        ...
        self._streams_container = Input(thermal_strain_Y._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._streams_container)
        self._data_sources = Input(thermal_strain_Y._spec().input_pin(4), 4, op, -1) 
        self._inputs.append(self._data_sources)
        self._bool_rotate_to_global = Input(thermal_strain_Y._spec().input_pin(5), 5, op, -1) 
        self._inputs.append(self._bool_rotate_to_global)
        self._mesh = Input(thermal_strain_Y._spec().input_pin(7), 7, op, -1) 
        self._inputs.append(self._mesh)
        self._requested_location = Input(thermal_strain_Y._spec().input_pin(9), 9, op, -1) 
...
class OutputsThermalStrainY(_Outputs):
    """Intermediate class used to get outputs from thermal_strain_Y operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
