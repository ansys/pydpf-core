...
class erp_radiation_efficiency(Operator):
    """Compute the radiation efficiency (enhanced erp divided by classical erp)
      ...
      >>> op = dpf.operators.result.erp_radiation_efficiency(fields_container=my_fields_container,mesh=my_mesh,time_scoping=my_time_scoping,mass_density=my_mass_density,speed_of_sound=my_speed_of_sound)

      >>> # Get output data
      >>> result_fields_container = op.outputs.fields_container()"""
    def __init__(self, fields_container=None, mesh=None, time_scoping=None, mass_density=None, speed_of_sound=None, config=None, server=None):
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Compute the radiation efficiency (enhanced erp divided by classical erp)""",
                             map_input_pin_spec={
                                 0 : PinSpecification(name = "fields_container", type_names=["fields_container"], optional=False, document=""""""), 
                                 1 : PinSpecification(name = "mesh", type_names=["abstract_meshed_region","meshes_container"], optional=False, document="""the meshes region in this pin has to be boundary or skin mesh"""), 
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "erp_radiation_efficiency")
...
class InputsErpRadiationEfficiency(_Inputs):
    """Intermediate class used to connect user inputs to erp_radiation_efficiency operator
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Input(erp_radiation_efficiency._spec().input_pin(0), 0, op, -1) 
        self._inputs.append(self._fields_container)
        self._mesh = Input(erp_radiation_efficiency._spec().input_pin(1), 1, op, -1) 
        self._inputs.append(self._mesh)
        self._time_scoping = Input(erp_radiation_efficiency._spec().input_pin(2), 2, op, -1) 
        self._inputs.append(self._time_scoping)
        self._mass_density = Input(erp_radiation_efficiency._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._mass_density)
        self._speed_of_sound = Input(erp_radiation_efficiency._spec().input_pin(4), 4, op, -1) 
    ...
    def mesh(self):
        ...
        - pindoc: the meshes region in this pin has to be boundary or skin mesh
...
class OutputsErpRadiationEfficiency(_Outputs):
    """Intermediate class used to get outputs from erp_radiation_efficiency operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Output(erp_radiation_efficiency._spec().output_pin(0), 0, op) 
