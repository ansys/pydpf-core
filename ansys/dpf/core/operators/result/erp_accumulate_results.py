...
class erp_accumulate_results(Operator):
    """Compute the Equivalent Radiated Power (ERP) by panels and sum over the panels
      ...
      >>> op = dpf.operators.result.erp_accumulate_results(fields_container=my_fields_container,mesh=my_mesh,time_scoping=my_time_scoping,mass_density=my_mass_density,speed_of_sound=my_speed_of_sound,erp_type=my_erp_type,boolean=my_boolean,factor=my_factor)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Compute the Equivalent Radiated Power (ERP) by panels and sum over the panels""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "erp_accumulate_results")
...
#internal name: erp_accumulate_results
#scripting name: erp_accumulate_results
class InputsErpAccumulateResults(_Inputs):
    """Intermediate class used to connect user inputs to erp_accumulate_results operator
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Input(erp_accumulate_results._spec().input_pin(0), 0, op, -1) 
        self._inputs.append(self._fields_container)
        self._mesh = Input(erp_accumulate_results._spec().input_pin(1), 1, op, -1) 
        self._inputs.append(self._mesh)
        self._time_scoping = Input(erp_accumulate_results._spec().input_pin(2), 2, op, -1) 
        self._inputs.append(self._time_scoping)
        self._mass_density = Input(erp_accumulate_results._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._mass_density)
        self._speed_of_sound = Input(erp_accumulate_results._spec().input_pin(4), 4, op, -1) 
...
class OutputsErpAccumulateResults(_Outputs):
    """Intermediate class used to get outputs from erp_accumulate_results operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
