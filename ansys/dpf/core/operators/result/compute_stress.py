...
All units need to be consistent.
      ...
      >>> op = dpf.operators.result.compute_stress(scoping=my_scoping,streams_container=my_streams_container,data_sources=my_data_sources,requested_location=my_requested_location,strain=my_strain)
...
All units need to be consistent.""",
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "compute_stress")
...
class InputsComputeStress(_Inputs):
    """Intermediate class used to connect user inputs to compute_stress operator
...
class OutputsComputeStress(_Outputs):
    """Intermediate class used to get outputs from compute_stress operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
