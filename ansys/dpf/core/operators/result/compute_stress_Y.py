...
All units need to be consistent.Get the YY normal component (11 component).
      ...
      >>> op = dpf.operators.result.compute_stress_Y(scoping=my_scoping,streams_container=my_streams_container,data_sources=my_data_sources,requested_location=my_requested_location,strain=my_strain)
...
class InputsComputeStressY(_Inputs):
    """Intermediate class used to connect user inputs to compute_stress_Y operator
    ...
    def __init__(self, op: Operator):
        ...
        self._streams_container = Input(compute_stress_Y._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._streams_container)
        self._data_sources = Input(compute_stress_Y._spec().input_pin(4), 4, op, -1) 
        self._inputs.append(self._data_sources)
        self._requested_location = Input(compute_stress_Y._spec().input_pin(9), 9, op, -1) 
...
class OutputsComputeStressY(_Outputs):
    """Intermediate class used to get outputs from compute_stress_Y operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
