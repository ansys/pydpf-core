...
class part_eroded_kinetic_energy(Operator):
    """Read Part Eroded Kinetic Energy (LSDyna) by calling the readers defined by the datasources.
      ...
      >>> op = dpf.operators.result.part_eroded_kinetic_energy(data_sources=my_data_sources,entity_scoping=my_entity_scoping)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Read Part Eroded Kinetic Energy (LSDyna) by calling the readers defined by the datasources.""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "M_ERKE")
...
class InputsPartErodedKineticEnergy(_Inputs):
    """Intermediate class used to connect user inputs to part_eroded_kinetic_energy operator
    ...
    def __init__(self, op: Operator):
        ...
        self._streams_container = Input(part_eroded_kinetic_energy._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._streams_container)
        self._data_sources = Input(part_eroded_kinetic_energy._spec().input_pin(4), 4, op, -1) 
        self._inputs.append(self._data_sources)
        self._entity_scoping = Input(part_eroded_kinetic_energy._spec().input_pin(6), 6, op, -1) 
...
class OutputsPartErodedKineticEnergy(_Outputs):
    """Intermediate class used to get outputs from part_eroded_kinetic_energy operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Output(part_eroded_kinetic_energy._spec().output_pin(0), 0, op) 
