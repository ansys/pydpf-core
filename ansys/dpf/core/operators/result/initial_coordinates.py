...
class initial_coordinates(Operator):
    """Read/compute Initial Coordinates (LSDyna) by calling the readers defined by the datasources.
      ...
      >>> op = dpf.operators.result.initial_coordinates(time_scoping=my_time_scoping,mesh_scoping=my_mesh_scoping,data_sources=my_data_sources)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Read/compute Initial Coordinates (LSDyna) by calling the readers defined by the datasources.""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "XI")
...
class InputsInitialCoordinates(_Inputs):
    """Intermediate class used to connect user inputs to initial_coordinates operator
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Input(initial_coordinates._spec().input_pin(2), 2, op, -1) 
        self._inputs.append(self._fields_container)
        self._streams_container = Input(initial_coordinates._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._streams_container)
        self._data_sources = Input(initial_coordinates._spec().input_pin(4), 4, op, -1) 
        self._inputs.append(self._data_sources)
        self._bool_rotate_to_global = Input(initial_coordinates._spec().input_pin(5), 5, op, -1) 
...
class OutputsInitialCoordinates(_Outputs):
    """Intermediate class used to get outputs from initial_coordinates operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Output(initial_coordinates._spec().output_pin(0), 0, op) 
