...
class beam_s_shear_force(Operator):
    """Read Beam S Shear Force (LSDyna) by calling the readers defined by the datasources.
      ...
      >>> op = dpf.operators.result.beam_s_shear_force(time_scoping=my_time_scoping,mesh_scoping=my_mesh_scoping,data_sources=my_data_sources)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Read Beam S Shear Force (LSDyna) by calling the readers defined by the datasources.""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "B_T1")
...
class InputsBeamSShearForce(_Inputs):
    """Intermediate class used to connect user inputs to beam_s_shear_force operator
    ...
    def __init__(self, op: Operator):
        ...
        self._streams_container = Input(beam_s_shear_force._spec().input_pin(3), 3, op, -1) 
...
class OutputsBeamSShearForce(_Outputs):
    """Intermediate class used to get outputs from beam_s_shear_force operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
