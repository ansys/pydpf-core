...
Get the YZ shear component (12 component).
      ...
      >>> op = dpf.operators.result.compute_total_strain_YZ(time_scoping=my_time_scoping,scoping=my_scoping,streams_container=my_streams_container,data_sources=my_data_sources,extrapolate=my_extrapolate,nonlinear=my_nonlinear,meshed_region=my_meshed_region,requested_location=my_requested_location,displacement=my_displacement)
...
class InputsComputeTotalStrainYZ(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._time_scoping = Input(compute_total_strain_YZ._spec().input_pin(0), 0, op, -1) 
        self._inputs.append(self._time_scoping)
        self._scoping = Input(compute_total_strain_YZ._spec().input_pin(1), 1, op, -1) 
        self._inputs.append(self._scoping)
        self._streams_container = Input(compute_total_strain_YZ._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._streams_container)
        self._data_sources = Input(compute_total_strain_YZ._spec().input_pin(4), 4, op, -1) 
        self._inputs.append(self._data_sources)
        self._extrapolate = Input(compute_total_strain_YZ._spec().input_pin(5), 5, op, -1) 
        ...
        self._meshed_region = Input(compute_total_strain_YZ._spec().input_pin(7), 7, op, -1) 
        self._inputs.append(self._meshed_region)
        self._requested_location = Input(compute_total_strain_YZ._spec().input_pin(9), 9, op, -1) 
        self._inputs.append(self._requested_location)
        self._displacement = Input(compute_total_strain_YZ._spec().input_pin(10), 10, op, -1) 
...
class OutputsComputeTotalStrainYZ(_Outputs):
    """Intermediate class used to get outputs from compute_total_strain_YZ operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
