...
class part_momentum(Operator):
    """Read Part Momentum (LSDyna) by calling the readers defined by the datasources.
      ...
      >>> op = dpf.operators.result.part_momentum(data_sources=my_data_sources,entity_scoping=my_entity_scoping)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Read Part Momentum (LSDyna) by calling the readers defined by the datasources.""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "M_MV")
...
class OutputsPartMomentum(_Outputs):
    """Intermediate class used to get outputs from part_momentum operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
