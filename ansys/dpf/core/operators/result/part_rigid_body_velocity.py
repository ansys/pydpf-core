...
class part_rigid_body_velocity(Operator):
    """Read Part Rigid Body Velocity (LSDyna) by calling the readers defined by the datasources.
      ...
      >>> op = dpf.operators.result.part_rigid_body_velocity(data_sources=my_data_sources,entity_scoping=my_entity_scoping)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Read Part Rigid Body Velocity (LSDyna) by calling the readers defined by the datasources.""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "M_RBV")
...
class InputsPartRigidBodyVelocity(_Inputs):
    """Intermediate class used to connect user inputs to part_rigid_body_velocity operator
    ...
    def __init__(self, op: Operator):
        ...
        self._streams_container = Input(part_rigid_body_velocity._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._streams_container)
        self._data_sources = Input(part_rigid_body_velocity._spec().input_pin(4), 4, op, -1) 
        self._inputs.append(self._data_sources)
        self._entity_scoping = Input(part_rigid_body_velocity._spec().input_pin(6), 6, op, -1) 
...
class OutputsPartRigidBodyVelocity(_Outputs):
    """Intermediate class used to get outputs from part_rigid_body_velocity operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Output(part_rigid_body_velocity._spec().output_pin(0), 0, op) 
