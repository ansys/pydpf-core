...
class beam_axial_plastic_strain(Operator):
    """Read Beam Axial Plastic strain (LSDyna) by calling the readers defined by the datasources.
      ...
      >>> op = dpf.operators.result.beam_axial_plastic_strain(time_scoping=my_time_scoping,mesh_scoping=my_mesh_scoping,data_sources=my_data_sources,integration_point=my_integration_point)

      >>> # Get output data
      >>> result_fields_container = op.outputs.fields_container()"""
    def __init__(self, time_scoping=None, mesh_scoping=None, data_sources=None, integration_point=None, config=None, server=None):
        ...
        if integration_point !=None:
            self.inputs.integration_point.connect(integration_point)

    @staticmethod
    def _spec():
        spec = Specification(description="""Read Beam Axial Plastic strain (LSDyna) by calling the readers defined by the datasources.""",
                             map_input_pin_spec={
                                 ...
                                 6 : PinSpecification(name = "integration_point", type_names=["int32"], optional=True, document="""integration point where the result will be read from. Default value: 0 (first integration point).""")},
                             map_output_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "B_EPPL")
...
class InputsBeamAxialPlasticStrain(_Inputs):
    """Intermediate class used to connect user inputs to beam_axial_plastic_strain operator
    ...
    def __init__(self, op: Operator):
        ...
        self._time_scoping = Input(beam_axial_plastic_strain._spec().input_pin(0), 0, op, -1) 
        self._inputs.append(self._time_scoping)
        self._mesh_scoping = Input(beam_axial_plastic_strain._spec().input_pin(1), 1, op, -1) 
        self._inputs.append(self._mesh_scoping)
        self._streams_container = Input(beam_axial_plastic_strain._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._streams_container)
        self._data_sources = Input(beam_axial_plastic_strain._spec().input_pin(4), 4, op, -1) 
        self._inputs.append(self._data_sources)
        self._integration_point = Input(beam_axial_plastic_strain._spec().input_pin(6), 6, op, -1) 
    ...
    def integration_point(self):
        ...
        - pindoc: integration point where the result will be read from. Default value: 0 (first integration point).
...
class OutputsBeamAxialPlasticStrain(_Outputs):
    """Intermediate class used to get outputs from beam_axial_plastic_strain operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Output(beam_axial_plastic_strain._spec().output_pin(0), 0, op) 
