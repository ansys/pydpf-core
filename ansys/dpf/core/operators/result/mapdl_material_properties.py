...
class mapdl_material_properties(Operator):
    """Read the values of the properties of a material for a given materials property field(property field that contains materials information for each element of a mesh).It returns a fields container containing a field for each material property, with only one value per material.The following keys can be used: Young's modulus (keys: EX, EY, EZ), Poisson's ratio (keys: NUXY, NUYZ, NUXZ), Shear Modulus (keys: GXY, GYZ, GXZ),Coefficient of Thermal Expansion (keys: ALPX, ALPY, ALPZ), Volumic Mass (key: DENS), second Lame's coefficient (key: MU),Damping coefficient (key: DAMP), thermal Conductivity (keys: KXX, KYY, KZZ), Resistivity (keys: RSVX, RSVY, RSVZ),Specific heat in constant volume (key: C), Film coefficient (key: HF), Viscosity (key: VISC), Emissivity (key: EMIS).
      ...
      >>> op = dpf.operators.result.mapdl_material_properties(properties_name=my_properties_name,materials=my_materials,streams_container=my_streams_container,data_sources=my_data_sources)

      >>> # Get output data
      >>> result_properties_value = op.outputs.properties_value()"""
    def __init__(self, properties_name=None, materials=None, streams_container=None, data_sources=None, config=None, server=None):
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Read the values of the properties of a material for a given materials property field(property field that contains materials information for each element of a mesh).It returns a fields container containing a field for each material property, with only one value per material.The following keys can be used: Young's modulus (keys: EX, EY, EZ), Poisson's ratio (keys: NUXY, NUYZ, NUXZ), Shear Modulus (keys: GXY, GYZ, GXZ),Coefficient of Thermal Expansion (keys: ALPX, ALPY, ALPZ), Volumic Mass (key: DENS), second Lame's coefficient (key: MU),Damping coefficient (key: DAMP), thermal Conductivity (keys: KXX, KYY, KZZ), Resistivity (keys: RSVX, RSVY, RSVZ),Specific heat in constant volume (key: C), Film coefficient (key: HF), Viscosity (key: VISC), Emissivity (key: EMIS).""",
                             map_input_pin_spec={
                                 0 : PinSpecification(name = "properties_name", type_names=["string"], optional=False, document=""""""), 
                                 1 : PinSpecification(name = "materials", type_names=["property_field"], optional=False, document="""Property field that contains a material id per element."""), 
                                 3 : PinSpecification(name = "streams_container", type_names=["streams_container"], optional=False, document=""""""), 
                                 4 : PinSpecification(name = "data_sources", type_names=["data_sources"], optional=False, document="""""")},
                             map_output_pin_spec={
                                 0 : PinSpecification(name = "properties_value", type_names=["fields_container"], optional=False, document="""""")})
        return spec


    @staticmethod
    def default_config():
        return Operator.default_config(name = "mapdl_material_properties")
...
class InputsMapdlMaterialProperties(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._properties_name = Input(mapdl_material_properties._spec().input_pin(0), 0, op, -1) 
        self._inputs.append(self._properties_name)
        self._materials = Input(mapdl_material_properties._spec().input_pin(1), 1, op, -1) 
        self._inputs.append(self._materials)
        self._streams_container = Input(mapdl_material_properties._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._streams_container)
        self._data_sources = Input(mapdl_material_properties._spec().input_pin(4), 4, op, -1) 
    ...
    def materials(self):
        ...
        - pindoc: Property field that contains a material id per element.
...
class OutputsMapdlMaterialProperties(_Outputs):
    """Intermediate class used to get outputs from mapdl_material_properties operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self._properties_value = Output(mapdl_material_properties._spec().output_pin(0), 0, op) 
