...
class plastic_state_variable(Operator):
    """Read/compute element nodal plastic state variable by calling the readers defined by the datasources. Regarding the requested location and the input mesh scoping, the result location can be Nodal/ElementalNodal/Elemental.
      ...
      >>> op = dpf.operators.result.plastic_state_variable(time_scoping=my_time_scoping,mesh_scoping=my_mesh_scoping,data_sources=my_data_sources,requested_location=my_requested_location,read_beams=my_read_beams)
...
class InputsPlasticStateVariable(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._read_beams = Input(plastic_state_variable._spec().input_pin(21), 21, op, -1) 
