...
class elastic_strain_energy_density(Operator):
    """Read/compute element nodal elastic strain energy density by calling the readers defined by the datasources. Regarding the requested location and the input mesh scoping, the result location can be Nodal/ElementalNodal/Elemental.
      ...
      >>> op = dpf.operators.result.elastic_strain_energy_density(time_scoping=my_time_scoping,mesh_scoping=my_mesh_scoping,data_sources=my_data_sources,requested_location=my_requested_location,read_beams=my_read_beams)
...
class InputsElasticStrainEnergyDensity(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._read_beams = Input(elastic_strain_energy_density._spec().input_pin(21), 21, op, -1) 
