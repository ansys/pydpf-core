...
class total_contact_force(Operator):
    """Read/compute total contact force by calling the readers defined by the datasources.
      ...
      >>> op = dpf.operators.result.total_contact_force(time_scoping=my_time_scoping,mesh_scoping=my_mesh_scoping,data_sources=my_data_sources)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Read/compute total contact force by calling the readers defined by the datasources.""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "CFFTOT")
...
class InputsTotalContactForce(_Inputs):
    """Intermediate class used to connect user inputs to total_contact_force operator
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Input(total_contact_force._spec().input_pin(2), 2, op, -1) 
        self._inputs.append(self._fields_container)
        self._streams_container = Input(total_contact_force._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._streams_container)
        self._data_sources = Input(total_contact_force._spec().input_pin(4), 4, op, -1) 
        self._inputs.append(self._data_sources)
        self._bool_rotate_to_global = Input(total_contact_force._spec().input_pin(5), 5, op, -1) 
...
class OutputsTotalContactForce(_Outputs):
    """Intermediate class used to get outputs from total_contact_force operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Output(total_contact_force._spec().output_pin(0), 0, op) 
