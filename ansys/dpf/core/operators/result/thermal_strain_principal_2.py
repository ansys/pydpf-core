...
class thermal_strain_principal_2(Operator):
    """Read/compute element nodal component thermal strains 2nd principal component by calling the readers defined by the datasources and computing its eigen values.
      ...
      >>> op = dpf.operators.result.thermal_strain_principal_2(time_scoping=my_time_scoping,mesh_scoping=my_mesh_scoping,data_sources=my_data_sources)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Read/compute element nodal component thermal strains 2nd principal component by calling the readers defined by the datasources and computing its eigen values.""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "ETH2")
...
class InputsThermalStrainPrincipal2(_Inputs):
    """Intermediate class used to connect user inputs to thermal_strain_principal_2 operator
    ...
    def __init__(self, op: Operator):
        ...
        self._time_scoping = Input(thermal_strain_principal_2._spec().input_pin(0), 0, op, -1) 
        self._inputs.append(self._time_scoping)
        self._mesh_scoping = Input(thermal_strain_principal_2._spec().input_pin(1), 1, op, -1) 
        self._inputs.append(self._mesh_scoping)
        self._fields_container = Input(thermal_strain_principal_2._spec().input_pin(2), 2, op, -1) 
        self._inputs.append(self._fields_container)
        self._streams_container = Input(thermal_strain_principal_2._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._streams_container)
        self._data_sources = Input(thermal_strain_principal_2._spec().input_pin(4), 4, op, -1) 
        self._inputs.append(self._data_sources)
        self._bool_rotate_to_global = Input(thermal_strain_principal_2._spec().input_pin(5), 5, op, -1) 
        self._inputs.append(self._bool_rotate_to_global)
        self._mesh = Input(thermal_strain_principal_2._spec().input_pin(7), 7, op, -1) 
        self._inputs.append(self._mesh)
        self._requested_location = Input(thermal_strain_principal_2._spec().input_pin(9), 9, op, -1) 
        self._inputs.append(self._requested_location)
        self._read_cyclic = Input(thermal_strain_principal_2._spec().input_pin(14), 14, op, -1) 
        self._inputs.append(self._read_cyclic)
        self._read_beams = Input(thermal_strain_principal_2._spec().input_pin(22), 22, op, -1) 
...
class OutputsThermalStrainPrincipal2(_Outputs):
    """Intermediate class used to get outputs from thermal_strain_principal_2 operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Output(thermal_strain_principal_2._spec().output_pin(0), 0, op) 
