...
class beam_axial_force(Operator):
    """Read Beam Axial Force (LSDyna) by calling the readers defined by the datasources.
      ...
      >>> op = dpf.operators.result.beam_axial_force(time_scoping=my_time_scoping,mesh_scoping=my_mesh_scoping,data_sources=my_data_sources)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Read Beam Axial Force (LSDyna) by calling the readers defined by the datasources.""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "B_N")
...
class InputsBeamAxialForce(_Inputs):
    """Intermediate class used to connect user inputs to beam_axial_force operator
    ...
    def __init__(self, op: Operator):
        ...
        self._streams_container = Input(beam_axial_force._spec().input_pin(3), 3, op, -1) 
...
class OutputsBeamAxialForce(_Outputs):
    """Intermediate class used to get outputs from beam_axial_force operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
