...
class global_internal_energy(Operator):
    """Read/compute Global Internal Energy (LSDyna) by calling the readers defined by the datasources. The location will be overall (global result).
      ...
      >>> op = dpf.operators.result.global_internal_energy(time_scoping=my_time_scoping,data_sources=my_data_sources)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Read/compute Global Internal Energy (LSDyna) by calling the readers defined by the datasources. The location will be overall (global result).""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "GLOB_ENG_IE")
...
class InputsGlobalInternalEnergy(_Inputs):
    """Intermediate class used to connect user inputs to global_internal_energy operator
    ...
    def __init__(self, op: Operator):
        ...
        self._time_scoping = Input(global_internal_energy._spec().input_pin(0), 0, op, -1) 
        self._inputs.append(self._time_scoping)
        self._streams_container = Input(global_internal_energy._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._streams_container)
        self._data_sources = Input(global_internal_energy._spec().input_pin(4), 4, op, -1) 
...
class OutputsGlobalInternalEnergy(_Outputs):
    """Intermediate class used to get outputs from global_internal_energy operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Output(global_internal_energy._spec().output_pin(0), 0, op) 
