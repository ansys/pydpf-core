...
class beam_torsional_moment(Operator):
    """Read Beam Torsional Moment (LSDyna) by calling the readers defined by the datasources.
      ...
      >>> op = dpf.operators.result.beam_torsional_moment(time_scoping=my_time_scoping,mesh_scoping=my_mesh_scoping,data_sources=my_data_sources)
    ...
    @staticmethod
    def _spec():
        spec = Specification(description="""Read Beam Torsional Moment (LSDyna) by calling the readers defined by the datasources.""",
                             map_input_pin_spec={
    ...
    @staticmethod
    def default_config():
        return Operator.default_config(name = "B_MT")
...
class InputsBeamTorsionalMoment(_Inputs):
    """Intermediate class used to connect user inputs to beam_torsional_moment operator
    ...
    def __init__(self, op: Operator):
        ...
        self._time_scoping = Input(beam_torsional_moment._spec().input_pin(0), 0, op, -1) 
        self._inputs.append(self._time_scoping)
        self._mesh_scoping = Input(beam_torsional_moment._spec().input_pin(1), 1, op, -1) 
        self._inputs.append(self._mesh_scoping)
        self._streams_container = Input(beam_torsional_moment._spec().input_pin(3), 3, op, -1) 
        self._inputs.append(self._streams_container)
        self._data_sources = Input(beam_torsional_moment._spec().input_pin(4), 4, op, -1) 
...
class OutputsBeamTorsionalMoment(_Outputs):
    """Intermediate class used to get outputs from beam_torsional_moment operator
      Examples
      --------
      >>> from ansys.dpf import core as dpf
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container = Output(beam_torsional_moment._spec().output_pin(0), 0, op) 
