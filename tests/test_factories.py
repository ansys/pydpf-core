...
def test_over_time_freq_fields_container_1():
    ...
    assert labels == ["time"]
    assert len(fc) == 2
    f1_result = fc.get_fields_by_time_complex_ids(1)
    f2_result = fc.get_fields_by_time_complex_ids(2)
...
def test_over_time_freq_complex_fields_container_1():
    ...
    assert len(fc) == 4
    f1_result = fc.get_fields_by_time_complex_ids(1, 0)
    f2_result = fc.get_fields_by_time_complex_ids(2, 0)
    f1_result_im = fc.get_fields_by_time_complex_ids(1, 1)
    f2_result_im = fc.get_fields_by_time_complex_ids(2, 1)
    assert f1_result.location == locations.nodal
    assert f2_result.location == locations.nodal
    ...
    f1_result_im = fc.get_imaginary_fields(1)
    f2_result_im = fc.get_imaginary_fields(2)
    assert f1_result.location == locations.nodal
    assert f2_result.location == locations.nodal
    assert f1_result_im.location == locations.nodal
    assert f2_result_im.location == locations.nodal
...
def test_over_time_freq_complex_fields_container_2():
    ...
    f2_im = fields_factory.create_scalar_field(31)
    fc = fields_container_factory.over_time_freq_complex_fields_container({ 0.42 : f1, 1.10 : f2 }, 
...
def test_complex_fields_container():
    ...
    assert len(fc) == 2
    freal_result = fc.get_fields_by_time_complex_ids(complexid = 0)
    fim_result = fc.get_fields_by_time_complex_ids(complexid = 1)
...
def test_scoping_by_step_and_substep(plate_msup):
    ...
    scop = time_freq_scoping_factory.scoping_by_step_and_substep(1, 2, model.metadata.time_freq_support)
    assert scop is not None
    assert len(scop.ids) == 1
    assert scop.ids[0] == 2
    assert scop.location == locations.time_freq
