...
def test_custom_type_field(server_type_remote_process):
    ...
    op.connect(0, f)
    assert np.allclose(
        op.get_output(0, dpf.types.custom_type_field).data, [1000000000000, 200000000000000]
    )
...
def test_syntax_error(server_type_remote_process):
    ...
    dpf.load_library(
        dpf.path_utilities.to_server_os(
            os.path.join(current_dir, "tests", "testfiles", "pythonPlugins", "syntax_error_plugin"),
...
def test_custom_op_with_spec(server_type_remote_process):
    ...
    dpf.load_library(
        dpf.path_utilities.to_server_os(
            os.path.join(current_dir, "tests", "testfiles", "pythonPlugins"),
            server_type_remote_process,
        ),
